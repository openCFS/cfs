...
public:
  ...
  inline void writeVolume(char* filename, MultilevelGrid& grid);
...
//
inline void
MeshWriter::writeVolume(char* filename,MultilevelGrid& grid)
{
  ofstream theFile;
  Zaehler zaehler;
  ...
  grid.forEachVolumeElement(zaehler);
  int noOfElement = zaehler.getCounter();
  theFile << noOfElement << '\n';

  grid.forEachVolumeElement(writeElement);
