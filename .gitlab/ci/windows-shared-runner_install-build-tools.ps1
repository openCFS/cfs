...
# we use curl.exe and not curl for powershell as only curl.exe has the proper options and we don't use wget.exe which is available on Windows by default

# install ccache (https://ccache.dev/download.html)
curl.exe -sS --quiet https://github.com/ccache/ccache/releases/download/v4.10.2/ccache-4.10.2-windows-x86_64.zip
...
curl.exe -sS https://registrationcenter-download.intel.com/akdlm/IRC_NAS/a37c30c3-a846-4371-a85d-603e9a9eb94c/intel-oneapi-hpc-toolkit-2025.0.1.48_offline.exe -o oneapi_offline.exe
# verify that the stuff could be downloaded from Intel
...
# install git
curl.exe -sS https://github.com/git-for-windows/git/releases/download/v2.43.0.windows.1/PortableGit-2.43.0-64-bit.7z.exe
