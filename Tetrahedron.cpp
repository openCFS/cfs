...
bool
Tetrahedron::queryChildrenMarks()
{
  int i;
  int counter = 0;
...
//    for (ch = 0; ch < 5; ch++)
//    {
//      int noOfFaces = child[ch]->getNoOfFaces();
//      grd::Element* p = child[ch]->getParent();
//      for (int i = 0; i < noOfFaces; i++)
//      {
//        grd::Element* n = child[ch]->getNeighbor(i);
//        if (n)
//        {
//          int type = n->type();
//          if (type == GRD_TRIANGLE)
...
// Description
// generate irregular elements
// returns number of elements
void
Tetrahedron::close(int refPattern,ConformingClosure& closure)
{
  //int refPattern;
  int noOfElem;
  int i0,i1,i2,i3;
  ...
  // Init element buffer
  closure.init();
  ConformingClosure::TET theElem = closure.beginTetrahedron();
...
// case: three edges on a face
    ...
    case 7:
      ...
      noOfElem = 4;
      break;
// case: three edges not on a face
    case 11:
      i0 = v[0]->getId();
      i1 = v[1]->getId();
      i3 = v[3]->getId();
...
//
void
Tetrahedron::close(ConformingClosure& closure)
{
...
// Description
// only child in middle of face (octahedron) is of interest
bool
Tetrahedron::queryChildrenRefinement()
{
  if(isRefined()) {
    if (child[4]->isRefined())
      return true;
  }
...
//
bool
Tetrahedron::queryEdgeRefinement()
{
  for (int i = 0; i < 6; i++)
    if (e[i]->isRefined())
      return true;
...
//
bool
Tetrahedron::queryChildrenEdgeRefinement()
{
  int i;
  for (i = 0; i < 6; i++)
...
// Description
int
Tetrahedron::getCommonFace(Element* theNeighbor)
{
...
//
void
Tetrahedron::getRefElements(int face,Vertex** theVertex,Edge** theEdge,Element**
 theChild,int* theChildFace)
{
...
//
void
Tetrahedron::setVertices(Vertex** vt)
{
  for (int i = 0; i < 4; i++)
    v[i] = vt[i];
}
...
//
void
Tetrahedron::getVertices(Vertex** vt)
{
  for (int i = 0; i < 4; i++)
    vt[i] = v[i];
}
...
//
Element*
Tetrahedron::getNeighbor(int face)
{
  return n[face];
}
...
//
int
Tetrahedron::noOfVertexAtFace(int fc)
{
  return 3;
}
...
//
Vertex*
Tetrahedron::getVertexAtFace(int fc,int vt)
{
...
//
Vertex*
Tetrahedron::getVertexAtEdge(int ed,int vt)
{
...
//
Edge*
Tetrahedron::getEdgeAtFace(int fc,int ed)
{
