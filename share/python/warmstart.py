...
def process_file(file, args, recursive):
  ...
  if 'difficulties' in msg:
    ...
    level = ord('a') - ord(base[-1]) if base[-2] == '_' and base[-1] >= 'a' and base[-1] <= 'z' else -1

    if level >= args.max-1:
      if args.verbose:
        print("skip '" + file + "' which is already in repeat level " + str(level))
      return
    ...
    if not recursive and args.bound:
    ...
    if args.info:
      if level == 0:
        process_file(base[:-2] + '.info.xml', args, recursive=True) # base is ..._a
      if level > 0:  
        process_file(base[:-1] + chr(ord('ord') + level-1) + '.info.xml', args, recursive=True)
        
      print(('iter:' if not recursive else '  <<:') + '{:3}'.format(iter) + ' ' + cost + ': {:8.4f}'.format(value) + " issue: '" + msg[msg.find('- ')+2:] + "' problem: '" + file + "'")
    else:          
      if args.qsub:
...
parser.add_argument('--max', help="the maximal number for warmstart levels", type=int, default = 26)
...
for file in args.input:
  process_file(file, args, recursive=False)
