...
def create_lbm2d(resolution, case, inclusion, inclusion_size): 
  ...
  if case == 'pipe_bend': 
    ...
    mesh.ne.append(('inlet',range(nx,nx*(ny-1),nx)))
    mesh.ne.append(('outlet',range(int(0.3*nx*ny+nx-1),int(0.7*nx*ny+nx-1),nx)))
  return mesh
...
def create_mesh_from_gmsh(meshfile,type):
  ...
  if type == "apod6":
    mesh = two_scale_tools.create_mesh_for_apod6(meshfile,nodes,elem)
  elif type == "aux_cells":
    mesh = two_scale_tools.create_mesh_for_aux_cells(meshfile,nodes,elem)
  else:
