...
execute("./calculate-crosses.py " +str(stp)+' '+str(dim)+' '+ str(res)+' '+str(folder)+ ' --hom ' + str(mesh)+' --shape '+str(shape)+ ' --filter '+str(filt)+' --void_material '+str(void)+' --design '+args.design)
os.chdir(str(pwd)+'/'+str(folder))
execute("bash jobs")
os.chdir(str(pwd))
execute("./evaluate.py "+str(stp)+' '+str(dim)+' '+str(folder)+' --design '+args.design)
