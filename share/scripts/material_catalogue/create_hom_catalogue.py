...
if args.big:
    execute("python evaluate.py "+str(stp)+' '+str(dim)+' '+str(res)+' '+str(folder)+ ' --big '+str(args.big))
else:
  os.chdir(str(pwd)+'/'+str(folder))
  execute("bash jobs")
  os.chdir(str(pwd))
  execute("python evaluate.py "+str(stp)+' '+str(dim)+' '+str(folder)+' --design '+args.design)