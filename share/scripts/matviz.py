...
def read_stiff_angle(hdf_file, dim_2D, args):
  ...
  if args.parametrization == 'hom_rect':
    ...
    sh1 = get_element(f, "design_shear1_" + args.hom_access, args.h5_region, args.h5_step) if args.show == "hom_sheared_rot_cross" else sh1
...
def perform(args, h5_read, dim_2D, tensor, centers, aux_code, force_scale=None):
  ...
  if h5_read or dim_2D:
    ...
    viz = None
    if args.show == "hom_rect" or args.show == "hom_rot_cross" or args.show == "hom_sheared_rot_cross" or args.show == "rot" or args.show == 'stream' or args.show == 'hom_rect_mod':
...
parser.add_argument("--show", help="mode within boebbale, hom_rect or streamline", choices=['ortho_norm', 'mono_norm', 'ortho_err', 'hom_rect', 'hom_rot_cross', 'hom_sheared_rot_cross', 'rot', 'stream', 'hom_rect_mod'])
