...
# def create_3d_mesh_unstructured_2(coords, nondes_coords, nodes_force, nodes_support, st1, st2, st3, angles, ip_nx, ip_ny, ip_nz, grad, scale):
#   mesh = Mesh()
#   centers, min, max = coords[0:3]  # design nodes
# #   print 'min = ' + repr(min)
# #   print 'max = ' + repr(max)
...
#   # non-design region points are added to design region points in order to create a structured mesh
#   for i in range(len(nondes_centers)):
#     centers.append((nondes_centers[i]))
#   coords_old = coords
#   coords = (centers, (numpy.min((min[0], nondes_min[0])), numpy.min((min[1], nondes_min[1])), numpy.min((min[2], nondes_min[2]))), (numpy.max((max[0], nondes_max[0])), numpy.max((max[1], nondes_max[1])), numpy.max((max[2], nondes_max[2]))))
#   centers, min, max = coords[0:3] 
# 
#   # add pseudo stiffnesses to stiffnesses st1,st2,st3 of the design domain
#   # pseudo stiffness values for the non-design nodes are only used for the choice whether node is design or non-design value
#   st1 = numpy.append(st1, -50. * ones((len(nondes_centers), 1)))
#   st2 = numpy.append(st2, -50. * ones((len(nondes_centers), 1)))
#   st3 = numpy.append(st3, -50. * ones((len(nondes_centers), 1)))
# #   print 'shape angle = ' + repr(len(angles))
# #   print 'angles = ' + repr(angles[0])
#   
#   # add pseudo angles for non-design region
#   if angles <> None:
# #     print 'angles = ' + repr(angles.shape)
#     ang = zeros(((angles.shape)[0] + len(nondes_centers), 3))
#     for i in range((ang.shape)[0]):
#       if i < (angles.shape)[0]: 
#         ang[i, :] = angles[i, :]
#       else:
#         ang[i, :] = zeros((1, 3))        
# #   print 'angles = ' + repr(ang.shape)
#   
#   # calculate new dimension and number of elements for 
#   delta = (abs(max[0] - min[0]), abs(max[1] - min[1]), abs(max[2] - min[2]))
#   dx = float(delta[0] / ip_nx)  # (2.*ip_nx)
#   dy = float(delta[1] / ip_ny)
#   dz = float(delta[2] / ip_nz)
#   nx = int(delta[0] / dx)
#   ny = int(delta[1] / dy)
...
#   print 'ny = ' + repr(ny)
#   print 'nz = ' + repr(nz)   
#   # insert all structured element nodes into the mesh
#   idx = 0
#   for z in range(nz + 1):
#     for y in range(ny + 1):
#       for x in range(nx + 1):
...
#         idx += 1
#   # interpolate         
#   ip_data, ip_near, out, ndim = get_interpolation_unstructured(coords, grad, st1, st2, st3, nx, ny, nz, dx, dy, dz, ang)
#   within = 0
#   invalid = 0     
# #   print 's1_intp = '
# #   for i in range(len(out)):
# #     print repr(ip_data[i, 0])
# #   print 'out = ' + repr(out[:])
# #   print 'out_length ' + repr(len(out))
...
#         i += 1
#     
#   # map load/support nodes from unstructured grid on the structured grid
#   # map_bc_to_node(mesh, nodes_force, ip_data, min, max, 'load', dx, dy, dz, nx, ny, nz)    
#   # map_bc_to_node(mesh, nodes_force, nondes_ip_data, nondes_min, nondes_max, 'load', dx, dy, dz, nx, ny, nz)    
#   # map_bc_to_node(mesh, nodes_support, ip_data, min, max, 'support', dx, dy, dz, nx, ny, nz)    
#   # map_bc_to_node(mesh, nodes_support, nondes_ip_data, nondes_min, nondes_max, 'support', dx, dy, dz, nx, ny, nz)
#   if grad <> 'nearest':  
#     print str(within) + ' elements out of ' + str(len(out)) + ' in convex hull'  
#   if invalid > 0:  
#     print str(invalid) + ' elements out of ' + str(len(out)) + ' are considered invalid (shall not be visualized)'
