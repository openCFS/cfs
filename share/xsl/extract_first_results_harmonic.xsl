...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  <!-- This stylesheet extracts the results within info.xml from the first sequence step.
    We assume the harmonic case with complex results and give the result for the first frequency -->
...
<xsl:text>
    ...
    <!-- for every result take the first item -->
    <!-- start with the last item step_nr from any result -->
    <xsl:value-of select="/cfsInfo/analysis/harmonic/process/solveStep[1]/@currentFrequency"/><xsl:text> </xsl:text>
    <xsl:value-of select="/cfsInfo/calculation/process/sequence/result/item[1]/@step_nr"/><xsl:text> </xsl:text> 
    <xsl:for-each select="/cfsInfo/calculation/process/sequence">
      <xsl:for-each select="result">
