...
expressions:
\begin{itemize}
\item \myverb#<dirichletInhom># for the \attribute{value} and
\attribute{phase} attribute
\item \myverb#<load># for the \attribute{value} and \attribute{phase}
attribute
\item \myverb#<regionLoad># in the mechanic PDE for the attribute
\attribute{value}
\end{itemize}
...
The \element{input} element can be used to read in data on a whole region from
...
<pdename>
  <bcsAndLoads>
    <dirichletInhom name="region1" dof="x" value="input('inhom',1)"
                    phase="input('inhom',1)"/>
    <dirichletInhom name="region2" dof="y" value="input('inhom',1)"
                    phase="input('inhom',1)"/>
  </bcsAndLoads>
</pdename>
\end{verbatim}
...
\begin{center}
 ...
 \vspace{0.5cm}
  \begin{tabular}{lll}
    ...
    \midrule
    \myverb#t#     & Current time value       & \analysis{transient} analysis \\
    \myverb#f#     & Current frequency value  & \analysis{harmonic} analysis \\
...
\myverb#dynamics# keyword is by using directly the
\myverb#sin#-function. To define a variable with a sine of 1 kHz one can
simply write:
\begin{verbatim}
<bcsAndLoads>
...
<bcsAndLoads>
\end{verbatim}
The variable \myverb#t# reflects the current time value in a transient
simulation.
...
stays at 1 is written via the \myverb#if#-statement as follows:
\begin{verbatim}
<bcsAndLoads>
