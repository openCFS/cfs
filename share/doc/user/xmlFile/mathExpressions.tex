...
\begin{table}[ht]
  \begin{longtable}{c|c|c}
    ...
    \verb#pi#   or \verb#_pi# & $\pi = 3.1415926..$ & \\
...
\end{verbatim}
Sample data (2 columns) is read in from the files x\_amplitude.dat and x\_phase.dat.
Please note the type of quotes used!!!!
\newline
...
to use exactly same the same mesh as for the previous simulation, i.e.\
...
Dirichlet conditions, but may be supported for other BCs in the future.
...
\begin{table}[ht]
  \begin{longtable}{c|p{0.4\textwidth}|p{0.4\textwidth}}
    ...
    conditions \newline load \\
    \verb#r phi z# & Current local coordinate value & regionLoad
...
<bcsAndLoads>
\end{verbatim}
A ramp signal, which goes from $0 \ldots 1 s$ linear from $0 \ldots 1$ and then stays at 1 is written via the \verb#if#-statement as follows:
\begin{verbatim}
<bcsAndLoads>
