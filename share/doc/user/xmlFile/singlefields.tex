...
There is one surface region result. The calculated value is of type history and written to the \textit{info.xml} file or the filesystem if \xml{text} output is enabled.
...
There are two surface region results. The result is of type history and  written to the \textit{info.xml} file or to the filesystem if \xml{text} output is enabled.   
