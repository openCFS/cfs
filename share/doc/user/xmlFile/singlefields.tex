...
In the command  \attribute{region}, all areas in which the electric field has to
...
via the \topelement{domain} attribute \attribute{geometryType}, which allows the values \attrivalue{3d}, \attrivalue{plane}
and \attrivalue{axi} for its \attribute{geometryType} attribute. For a mechanic PDE this definition
must be repeated in the \analysis{mechanic} element via its \attribute{subtype} entry.
\begin{verbatim}
...
\emph{Note:} The \attribute{subType} value must of course match to the \attribute{geometryType}
...
There is one surface region result. The calculated value is of type history and written to the \textit{info.xml} file or the filesystem if \element{text} output is enabled.
...
\end{verbatim}
the displacements of the given direction are integrated over the given surface. The result is for harmonic analysis a complex value. Of interest is them the norm/length of the calculated vector.
...
The \analysis{acoustic} element is appended by the following child elements
\begin{itemize}
\item \element{region}
\item \element{surface}
\item \element{timeSteppingFormulation} effective stiffness formulation 
is default(optional), effective mass matrix formulation can be chosen unless one is not using the fractional damping model
\item \element{bcsAndLoads} (optional)
\item \element{nonLinear}\footnote{Other Nonlinearities than nonlinear acoustic
 PDE, which can be chosen by an attribute of \element{region} element. This is
 currently not implemented.}
\item \element{storeResults} (optional)
\end{itemize}
and the following attributes
\begin{itemize}
\item \element{pressFormu} used to choose for an acoustic pressure PDE formulatation \\
with \attrivalue{acouPressure} as value.
\item \attribute{subType} for flownoise computations set this to \attrivalue{combustionNoise} or \attrivalue{standard} 
\end{itemize}
...
region individually. Damping is a child element of \element{region},
...
frequency power law.
\attribute{nonLinear} comes as attribute to region. It can take the value \attrivalue{no},
\attrivalue{westervelt}, or \attrivalue{kuznetsov}.

\subsection{Boundary Conditions}
Unique to the acoustic PDE is the possibiliy to prescribe inhomogeneous Neumann boundary conditions. In acoustic potential formulation this means a normal velocity on a surface. Remember, that the specified surface elements have to appear in the \topelement{domain} section and in the \element{acoustic} section below the region definitions. The syntax is:
...
Analogously one can specify storing of certain node values with the help of the \element{nodeList}, by the sub element \subelement{nodes} and \attribute{name} attribute. 
...
There are two surface region results. The result is of type history and  written to the \textit{info.xml} file or to the filesystem if \attribute{text} output is enabled.   
...
\end{equation}
with $v$ the complex acoustic particle velocity and $p^*$ the complex conjugate acoustic pressure. Of special interest is the acoustic effective power
\begin{equation}
...
is needed, which defines the boundaries at which the \attrivalue{abc} are working.
...
called \element{rhsValues}. Here is an example:
...
The attribute \attribute{region} identifies the region on which the source term
is to be applied; it is always required. If there is no sub-element
\subelement{interpolation} then source terms are read from the file given through
the attribute \attribute{inputId} (which defaults to \attrivalue{default}).

Inside the \element{factor} element one can supply a mathematical expression
...
The \element{asyncSteps} element tackles that fact that time steps in CFD are
usually smaller than in computational acoustics. Setting this value to
\elemvalue{nearest} selects source terms from the time step that is nearest to the
one that is currently being computed. \element{interpolate} will result in a
...
CFS++, selecting data by step number, set attribute \attribute{justInterpolate} to attribute value \attrivalue{no} here.

The \element{interpolation} element tells CFS++ to interpolate source terms from
a different grid. This is required, if a CFD grid is not suitable for
acoustics. The attribute \attribute{justInterpolate} may be set to \attrivalue{yes} if
just the interpolation should be computed, but not the acoustic field. The
\element{srcRegions} element defines the CFD grid; \attribute{names} is the list
of regions (separated by space, \verb|;| or \verb#|#) that contain
source terms; \attribute{inputId} identifies the input file from which to read the
CFD grid and source terms; optionally \attribute{coordSysId} may specify the
coordinate system of the CFD grid if it is different from the acoustics grid.

The \element{xyPlane} element is only relevant if the acoustics grid is
two-dimensional, but the CFD grid is three-dimensional. Then one may specify
the \attribute{z} coordinate of the x-y-plane (defaults to 0). Additionally
\attribute{tol} gives the tolerance of \attribute{z} in meters, so that nodes in the
CFD grid need not lie exactly on the plane.

Inside the \element{tolerances} element one may specify a range for the global
...
\element{restartFileMode} determines if a restart file containing the
interpolation weights is to be read (\elemvalue{r}), written (\elemvalue{w}) or both
(\elemvalue{rw}). In this way the interpolation weights need to be computed only
once and can be reused if a simulation is restarted.

The \element{nodeWarnings} element defines how nodes are handled which could not
be mapped during interpolation. Any message is suppressed if the attribute
\attribute{display} is set to \attrivalue{no}. The total number of missed nodes is
reported, when set to \attrivalue{yes}. A message for each single node is generated
by a value of \attrivalue{verbose}. Optionally a text file containing the node
numbers is written, if the attribute \attribute{writeNodes} is set to \attrivalue{yes}.
...
coil needs an attribute {\attribute{name}="\attrivalue{xyz}"}, standing nearby the
...
amplitude of {\element{value}} is assumed.  
...
\noindent
with {\element direction} standing for one of 
\begin{itemize}
\item x
\item y
\item z
...
values {\attribute{orientX} }, {\attribute{orientY} } and {\attribute{orientZ} }
