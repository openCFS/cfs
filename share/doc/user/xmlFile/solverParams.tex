...
system OLAS. The necessary interface in CFS++ is provided by the \textbf{CFSOLASParams}
class.
...
done by inserting into the XML-file a \topelement{linearSystems}. Its position can be
seen in table \ref{tab:leavesLevelOne}. The \topelement{linearSystem} section has
...
\element{system} block must be inserted. The \attribute{name} attribute is used to
relate the linear system with the respective PDE. Thus, the value of \attribute{name}
must either be a PDE name, e.g. \attrivalue{mechanic}, or the keyword \attrivalue{direct}.
...
The \element{system} section itself can contain three child elements which can be
...
%
%
The first two children of the \element{system} element are the \subelement{setup} and
the \subelement{matrix} element. Both are used to specify parameters for the setup
of the linear system and the resulting matrix.

The \subelement{setup} element possesses the following two optional attributes:
\attribute{eqnNumbering} and \attribute{idbcHandling}.
\begin{center}
...
\vspace{0.5cm}
             & block  & This is relevant only for PDEs like e.g.~\pde{mechanic},
...
%
%
The \subelement{matrix} element possesses the following three optional attributes:
\attribute{entry}, \attribute{storage} and \attribute{reordering}.
%
%
\begin{center}
\begin{tabular}{llp{12cm}}
...
%
%
Note that not all values of \attribute{eqnNumbering}, \attribute{entry} and \attribute{storage}
...
\attrivalue{expertsChoice} is specified, which also is the default for the
...
handled (see \url{math.nist.gov/MatrixMarket} for both formats). The \textbf{MatrixMarket} format saves for each non-zero entry the row and column index and the \textbf{Harwell-Boeing} format stores the non-zero entries in a compressed column storage format, in the symmetric case the lower/upper triangular part is stored. Note that OLAS uses internally a compressed row
...
In order to trigger the export one must specify an \subelement{exportLinSys} element.
This is a child of the \element{system} element. It expects as \attribute{baseName}
attribute the basename for the two output files. The format is specified by
the \attribute{format} attribute. The default is \attrivalue{matrix-market} and files
with the extensions '.mtx' and '.vec' are created. The attribute value
\attrivalue{harwell-boeing} creates a file with '.hb' added to the base name.

Additionally in \subelement{exportLinSys} element, it is possible to write the solution vector via the attribute
\attribute{solution}. The possible values are \attrivalue{no} (default), \attrivalue{yes} and
\attrivalue{exclusive}. The later does not write the system matrix (and hence the
\attribute{format} attribute is ignored) but only the solution vector. This might be
...
\begin{itemize}
\item A simple but complete description of what to do with the system of a direct coupled PDE. The format is by default \attrivalue{matrix-market}, hence the files 'piezo.mm' and 'piezo.vec' with the RHS are written. 
\begin{verbatim}
<sequenceStep>
 ...
 <linearSystems>
...
\end{verbatim}
\item Compared to the example above, the solution vector is written to 'piezo.sol.vec'
\begin{verbatim}
<exportLinSys baseName="piezo" solution="yes" format="harwell-boeing"/>
\end{verbatim}
\item In this example only 'piezo.sol.vec' is created. This might be useful if one wants to compare
the solutions of different solvers (or solver settings). The \attribute{format} attribute applies only for the system and is therefore ignored. The value \attrivalue{matrix-market} would be default anyway.
\begin{verbatim}
...
preconditioning method by inserting a \subelement{solver} block into the \element{system}
section for the corresponding linear system.

The two choices for solver and preconditioner are specified as the \attribute{type}
and \attribute{precond} attributes of the \element{solver} element. The following two
...
\toprule
\multicolumn{2}{c}{Currently available choices for \attribute{type} attribute of
...
%
%
The default values for \attribute{type} and \attribute{precond} are \attrivalue{expertsChoice} and
\attrivalue{noPrecond}.
%
%

The \element{solver} element can have several children which can be used to specify
...
\begin{tabbing}
\quad\=\\ \kill
\myverb!<solver type="!\emph{value}\myverb!" precond="!\emph{value}\myverb!">!\\
\> \myverb!<stoppingRule type="!\emph{value}\myverb!"/>!\\
...
%
%
where \attrivalue{value} must be one of the possibilities from the list below:
%
%
\begin{center}
\begin{tabular}{lc}
...
In some cases choosing \attrivalue{relNormRHS} will lead to difficulties. One such
...
by OLAS automatically and \attrivalue{relNormRes0} will be used instead of
\attrivalue{relNormRHS}.
%
%
...
In order to specify parameters for solver \subelement{xyz} the \element{solver} element
is supplemented with a child element with name \childelement{xyz}. The parameters
...
All the solver parameters are optional as is the \element{xyz} element. If one or
...
\item If you use Pardiso as solver, it usually makes no sense to set
      the \attribute{reordering} attribute of the \element{solver} element to some other
      value besides the default of \attrivalue{noReordering}, unless you use
      \element{ordering} = \elemvalue{noReordering} in Pardiso.
\end{itemize}
%
%
...
%
%
The lapackLL solver currently only supports \childelement{logging}. Please also note
the following:
%
%
\begin{itemize}
...
\item Do not set \childelement{tryScaling} to yes, if you have inhomogeneous Dirichlet
...
saveFacFile     & string  & this parameter can be used to specify the name of
	                    a file to which the factor matrix $F=D+L^T$ is
                            ...
                            exported\\
\midrule
savePatternOnly & yes, no & if set to \elemvalue{yes}, then only the sparsity pattern
                            of the matrix is exported, but not the numerical
                            values of the factorisation\\
\midrule
logging         & yes, no & if set to \elemvalue{yes}, the solver will be verbose,
...
%
\subsection{CG}
Setting \attribute{type} to \attrivalue{cg} selects the native preconditioned conjugate
...
%
%
Setting \attribute{type} to \attrivalue{gmres} selects the OLAS implementation of the
...
\item If you have a positive definite system (real symmetric or Hermitian), you have to give this information explicitly, otherwise an indefinite symmetric system is assumed and some performance might be lost. The shortest version of \element{linearSystems} is then like the following:
\begin{verbatim}
<linearSystems>
  <system name="mechanic">
     <solver type="ilupack">
...
\end{verbatim}
\item If the iterative solver from the ILUPACK package exits too early, give a stricter \attribute{residualTol}. 
\begin{verbatim}
<solver type="ilupack">
   <ilupack>
...
\end{verbatim}
Check the .las file especially for the following lines. If the current residual norm is not as good as the target residual norm the solution is likely wrong.
\begin{verbatim}
...
...
\end{verbatim}
\item The memory consumption of ILUPACK, the fill-in factor, is set via \childelement{elbowSpace}.
\begin{verbatim}
<solver type="ilupack">
   <ilupack matrix="pd">
...
\end{tabular}
\end{center}
The theory behind is complex and we are still trying to understand it properly. Up to now we apply an heuristic approach and simply permutate
to find optimal settings. 

The parts to describe one of the \attrivalue{inital}, \attrivalue{regular} or \attrivalue{final} ordering are \attribute{matrix}, \childelement{ordering} and \childelement{matching}.

The \attribute{matrix} types are given above, note that this does not directly match the system matrix! E.g. might "gnl" (unsymmetric) be a good reordering matrix type for "pd" (symmetric positive definite) algebraic systems as defined as matrix attribute for the solver element.

The \childelement{ordering} values are in total the following, note that not all combinations really exist.
\begin{center}
\begin{tabular}{llp{0.5\textwidth}}
\toprule
...
The final part, the \childelement{matching} makes a maximum weight matching to improve the diagonal dominance using an external package.
\begin{center}
...
\item Here we do not give the matrix type of the \element{ilupack} element. Assuming the same
(symmetric) simulation as above, the default is \attrivalue{sym} for symmetric indefinite systems. 
\begin{verbatim}
<solver type="ilupack">
  <ilupack>
     <permutation type="regular" matrix="gnl" ordering="amd" />
...
\end{tabular}
\end{center}
Furthermode there is a set of flags which can optionally be set, again the .las file prints the default setting. This is done by adding one ore more \element{flag} elements to the \element{ilupack} element. Each element needs a \attribute{name}, possible value for attribute \attribute{name} are given below,
% and a \attribute{state} attribute. The flag states are \xml{on} and \xml{off}. Every user is invited to add knowlede about the flags (and all other parameters) to this document!
\begin{center}
\begin{tabular}{lp{0.5\textwidth}}
\toprule
flag & description \\
...
%
%
logging      & yes, no & if set to \elemvalue{yes}, the preconditioner will be
...
savePatternOnly & yes, no & if set to \elemvalue{yes}, then only the sparsity pattern
                            of the matrix is exported, but not the numerical
                            values of the factorisation\\
logging         & yes, no & if set to \elemvalue{yes}, the preconditioners will be
...
When Pardiso is available and you have a (free) license for your user and host (see \ref{sec:Pardiso}), Pardiso is often a good choice as it is quite fast and as a direct solver very stable and gains a high accuray. Countercheck in the .las file, if your system name is correct and the given solver is really used. The solution vector is written to 'piezo.sol.vec'. 
\begin{verbatim}
<linearSystems>
  <system name="direct">
...
\end{verbatim}
Giving more parameters one can try to increase performance. Like in the following example, where we explicitly express that the system is positive definite and we want logging to find hints for further tuning. Note that the pizeo example from above would not be positive definite.
\begin{verbatim}
<linearSystems>
  <system name="mechanic">
    <solver type="pardiso">
...
When the problems are large, the ILUPACK package (see \ref{sec:ilupack}) provides a set of preconditioners and iterative solvers which can give a far better performance than Pardiso. Note that the results for coupled problems should be checked very carefully and best performance is gained if features of the Pardiso library can be used. So in this example we again do not solve the coupled piezo problem.
\begin{verbatim}
<linearSystems>
  <system name="mechanic">
    <solver type="ilupack"/>
...
Finally we give an example where no external libraries are required and we entirely use self implemented code. The preconditioner is an incomplete $LU$ decomposition with no fill-in (see \ref{sec:Specifying_Solver_Parameters}), the solver is GMRES (a stable solver for indefinite problems).
\begin{verbatim}
<linearSystems>
