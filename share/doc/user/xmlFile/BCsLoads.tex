...
{\bf Inhomogeneous BCs}  are used for assigning a
  ...
  specific nodes (according to the section \element{nodes} in \topelement{$<$domain$>$}) and a value which has to be assigned to this node.
...
In the section  \topelement{$<$bcsAndLoads$>$} of the appropriate PDE in the
...
With inhomogeneous BCs, one has additionally to define the \attrivalue{value} by
...
the name of the time file in the attribute {\attrivalue{signal.dat} }:

\verb!<dirichletInHom name="nameBound3" value="10 * sample1D('signal.dat',t,1)"/>!
...
\noindent
has to be called. Herein, {\attrivalue{XXX} } stands for the name degrees of
...
<bcsAndLoads>
   ...
   <load           name="centerPoint" dof="x"  value="3.0"/>
</bcsAndLoads>
\end{verbatim}
...
charge.  The syntax of the {\element{load} } command is equal to that of 
