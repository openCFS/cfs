...
12.04. The script and accompanying binary packages\footnote{OSX cross-compiler
  build script  and binary  packages: \url{\CFSDSFTP/binaries/xcode}}  is also
...
server  at  \url{\CFSDSFTP/scripts/bootstrap_devel_machine.cmd}   or  in  {\tt
  share/scripts} inside the CFS++ source.   The script should be placed inside
a  directory, where  the  MinGW environment  is to  be  installed, e.g.   {\tt
  E:{\textbackslash}mingw\_env} cf.  Fig. \ref{fig:mingw_env_xp32}.  The drive
containing this folder should have about 12GB of free space.
