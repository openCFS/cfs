...
sudo zypper install subversion gcc gcc-c++ gcc-fortran automake autoconf \
                    ...
                    diffutils patch zip xorg-x11-libXp tk-devel
\end{verbatim}
...
sudo apt-get install subversion gcc g++ gfortran automake autoconf cmake \
                     ...
                     libgl1-mesa-dev libglu1-mesa-dev libxmuu-dev
\end{verbatim} 

\begin{verbatim}
ln -s /usr/include/python2.6 /usr/include/python
ln -s /usr/lib/libXext.so.6.4.0 /usr/lib/libXext.so
...
echo "baseurl=http://ftp.ucr.ac.cr/epel/5/$basearch" >> $YC

ARCH=$(uname -m | sed 's/i[0-9]86/i386/')
...
yum install subversion gcc gcc-c++ gcc-gfortran automake autoconf cmake \
            ...
            python-pygments doxygen tcl-devel python-devel git-svn \
            cmake-gui java-1.6.0-openjdk-devel tk-devel\
            patch diffutils zip libXt-devel libXp mesa-libGLU-devel libXmu-devel
            
ln -s /usr/lib64/libXext.so.6.4.0 /usr/lib64/libXext.so

echo "export JAVA_HOME=/usr" >> $HOME/.bashrc
echo "export PATH=/opt/svnkit-1.3.5.7406:$PATH" >> $HOME/.bashrc
\end{verbatim} 
...
refer to Sec. \ref{sec:gmv} for an introduction on using GMV.
