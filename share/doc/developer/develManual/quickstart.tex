...
for  Linux based  OSes on  the distribution.   We give  here the  commands for
installing the required packages from the command line.  The package lists are
provided for some  of the OSes most  often in use by the  CFS++ developers. If
you use a different distribution you should be able to figure out the required
...
sudo zypper install subversion gcc gcc-c++ gcc-fortran automake autoconf \
                    ...
                    diffutils patch zip xorg-x11-libXp
\end{verbatim}
...
sudo apt-get install subversion gcc g++ gfortran automake autoconf cmake \
                     ...
                     patch diffutils zip libxt-dev libxp6
\end{verbatim} 
...
\subsection{Fedora (13)}

\noindent As root user enter the following:
\begin{verbatim}
yum install subversion gcc gcc-c++ gcc-gfortran automake autoconf cmake \
            perl graphviz texlive-latex tetex-tex4ht \
            python-pygments doxygen tcl-devel python-devel git-svn \
            cmake-gui java-1.6.0-openjdk-devel \
            patch diffutils zip libXt-devel libXp
\end{verbatim} 
...
Here  is a shell  script \attachfile[icon=PushPin,description={Shell  script for
setting         up        a         Mac        OS         X        development
machine.},mimetype=text/plain]{attachments/mac_devel_setup.txt} , which guides
you through the setup process of a Mac OS X development machine. It was tested
on Snow Leopard.
...
built    executables   ({\tt    cfs\_nightly\_*,    cfstool\_nightly\_*})   in
Klagenfurt. For an introduction to  modeling refer to the chapter 'Basic Steps
of Modeling in ANSYS' in \cite{Simkovics2010}.
...
\url{http://www.paraview.org/Wiki/images/8/88/ParaViewTutorial38.pdf}.
