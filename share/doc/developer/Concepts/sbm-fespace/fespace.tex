...
\section{Motivation}
\begin{frame}[fragile]{Two-Level Solver for Nonlinear Problems}
\begin{itemize} 
...
\begin{frame}[fragile]{Two-Level Solver for Nonlinear Problems (cont.)}
 \begin{itemize}
...
\begin{frame}[fragile]{Solution strategy}
 \begin{enumerate}
...
\begin{enumerate}
 \item Comment out all PDEs and Forms except the simple elecPDE and the
linElecInt integrator to get a \emph{lightweitht} implementation playground
 \item Create FeSpace base class and FeSpaceH1Lo, which collects all the
pointers to the Lagrangian elements
