function varargout = eval_h5(f_path,varargin)
...
%   evaluates a .cfs result file from an CFS++ simulation. The input
...
elseif strcmp(return_procedure_post,'ret_array')
    varargout{3} = post_time_array;
    varargout{4} = post_val_array;
    varargout{5} = cont_out;
else
    warning('No valid return-procedure found, the default procedure will be applied (ret_array)')
    varargout{3} = post_time_array;
    varargout{4} = post_val_array;
