...
%EVAL_MECH (V0.1) this function evaluates all results considering mechancs
%and is also able to apply postprocessing to the simulation results
%ALPHA-STATE, just some basic functions are implemented!
switch nargin
    case 0
        %return the information needed by the main program
        info_cell = cell(3,1);
        info_cell{1} = 'mechDisplacement';
        info_cell{2} = 'mechVelocity';
        info_cell{3} = 'mechAcceleration';
        varargout{1} = info_cell;
        return
    case 1
        eval_type = varargin{1};
        switch eval_type
            case 'mechDisplacement'
                varargout{1} = 'Displacement (m)';
            case 'mechVelocity'
                varargout{1} = 'Velocity (m/s)';
            case 'mechAcceleration'
                varargout{1} = 'Acceleration (m/s²)';
        end
        return
    case 5
...
%COPY FROM EVAL_ACOU, DO NOT USE!!!
warning('COPY FROM EVAL_ACOU, DO NOT USE!!!')
switch postprocess
    case 'acouPressure'
