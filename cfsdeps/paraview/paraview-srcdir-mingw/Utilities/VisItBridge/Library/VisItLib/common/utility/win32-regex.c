     ...
     int stop)
{
  ...
  for (;;)
    ...
    case charset:
    case charset_not:{
      boolean GCC_DOES_NOT_LIKE_not = (re_opcode_t) *(p - 1) == charset_not;

        unsigned char c;
            DEBUG_PRINT2 ("EXECUTING charset%s.\n", GCC_DOES_NOT_LIKE_not ? "_not" : "");
        ...
        && p[1 + c / BYTEWIDTH] & (1 << (c % BYTEWIDTH)))
          GCC_DOES_NOT_LIKE_not = !GCC_DOES_NOT_LIKE_not;

        p += 1 + *p;

        if (!GCC_DOES_NOT_LIKE_not) goto fail;
    ...
    case on_failure_jump:
       ...
       We change it to either `pop_failure_jump' or `jump'.  */
        ...
        register unsigned char c
          ...
          {
            int GCC_DOES_NOT_LIKE_not = (re_opcode_t) p1[3] == charset_not;

            if (c < (unsigned char) (p1[4] * BYTEWIDTH)
            && p1[5 + c / BYTEWIDTH] & (1 << (c % BYTEWIDTH)))
              GCC_DOES_NOT_LIKE_not = !GCC_DOES_NOT_LIKE_not;

                    /* `GCC_DOES_NOT_LIKE_not' is equal to 1 if c would match, which means
                        that we can't change to pop_failure_jump.  */
            if (!GCC_DOES_NOT_LIKE_not)
                      {
                  p[-3] = (unsigned char) pop_failure_jump;
