...
vtkStandardNewMacro(vtkNACSReader)

const double pi = std::acos( -1.0 );

void vtkNACSReader::SetMultiSequenceStep( int step  ) {
...
void vtkNACSReader::SetTimeStep (unsigned int step) {
...
const char* vtkNACSReader::GetTimeFreqValStr() {
  return timeFreqValStr.c_str();
}

void vtkNACSReader::SetAddDimensionsToArrayNames(int flag) {
...
void vtkNACSReader::SetHarmonicDataAsModeShape(int flag) {
...
void vtkNACSReader::SetFillMissingResults(int flag) {
...
int vtkNACSReader::GetNumberOfRegionArrays() {
  return regionNames_.size();
}

int vtkNACSReader::GetRegionArrayStatus( const char * name ){
...
void vtkNACSReader::SetRegionArrayStatus( const char * name, int status ){
...
const char * vtkNACSReader::GetRegionArrayName( int index) {
  return regionNames_[index].c_str();
}

int vtkNACSReader::GetNumberOfNamedNodeArrays() {
  return namedNodeNames_.size();
}

int vtkNACSReader::GetNamedNodeArrayStatus( const char * name ){
...
void vtkNACSReader::SetNamedNodeArrayStatus( const char * name, int status ){
...
const char * vtkNACSReader::GetNamedNodeArrayName( int index) {
  return namedNodeNames_[index].c_str();
}

int vtkNACSReader::GetNumberOfNamedElemArrays() {
  return namedElemNames_.size();
}

int vtkNACSReader::GetNamedElemArrayStatus( const char * name ){
...
void vtkNACSReader::SetNamedElemArrayStatus( const char * name, int status ){
...
const char * vtkNACSReader::GetNamedElemArrayName( int index) {
  return namedElemNames_[index].c_str();
}

//----------------------------------------------------------------------------
vtkNACSReader::vtkNACSReader()
{
  this->FileName = NULL;
...
int vtkNACSReader::CanReadFile(const char* fname) {
  
  // Try to "load" the file (which internally just opens the mesh group)
  // to see, if this is a true NACS HDF5 file. 
  std::string fileName = std::string(fname);
  H5NACS::Hdf5Reader temp; 
  try {
    temp.LoadFile( fileName );
    // If this succeeds, we assume that this is a true NACS HDF5 file.
    return true;
  } catch (...) {
    return false;
  }
...
//----------------------------------------------------------------------------
int vtkNACSReader::RequestInformation(
  ...
  vtkInformationVector *outputVector)
{
 ...
 try {
   ...
   if( this->msStepChanged ) {
     ...
     std::map<unsigned int,H5NACS::AnalysisType> analysis;
     ...
     std::vector<shared_ptr<H5NACS::ResultInfo> >infos;

     if( analysis.size() > 0 ) {

       // store current analsis type
       this->analysisType = analysis[MultiSequenceStep]; 
       if( this->analysisType == H5NACS::HARMONIC || 
           this->analysisType == H5NACS::EIGENFREQUENCY) {
         harmonicData = true;
       } else {
         harmonicData = false;
       }
...
//----------------------------------------------------------------------------
void vtkNACSReader::PrintSelf(ostream& os, vtkIndent indent)
{
...
//----------------------------------------------------------------------------
void vtkNACSReader::ReadFile(vtkMultiBlockDataSet *output)
{
...
//----------------------------------------------------------------------------
void vtkNACSReader::UpdateActiveRegions() {
...
//----------------------------------------------------------------------------
void vtkNACSReader::ReadNodeCellData(vtkMultiBlockDataSet *output, bool isNode)
{
  
  DBG_OUT("=========================");
  DBG_OUT( " Reading Node/Cell Data" );
  DBG_OUT("=========================");
  H5NACS::EntityType entType = H5NACS::ELEMENT;
  ...
  try {
    ...
    std::map<std::string, shared_ptr<H5NACS::ResultInfo> > nameInfo;
    for( unsigned int i = 0; i < infos.size(); ++i ) {
    ...
    std::vector<std::vector<shared_ptr<H5NACS::ResultInfo> > > regionInfos;
    shared_ptr<H5NACS::Result> result;
    ...
    for( ; it != resultNames.end(); ++it ) {
      ...
      if( ( isNode && (nameInfo[resultName]->listType != H5NACS::NODE ) )
          ||(!isNode && (nameInfo[resultName]->listType != H5NACS::ELEMENT ) ) )
        continue;
      ...
      for( unsigned int iRegion = 0; iRegion < regionNames_.size(); iRegion++ ) {
        ...
        if( found ) {
          result = shared_ptr<H5NACS::Result>(new H5NACS::Result());
          ...
          isComplex = (this->analysisType == H5NACS::HARMONIC);
        }
        ...
        if( !isComplex ) {
          ...
          if( !HarmonicDataAsModeShape ) {
            ...
            if( ComplexMode == 0 ) {
              ...
              vals1 = SaveToArray( ampl, numDofs, numEntities, amplName );
              vals2 = SaveToArray( phase, numDofs, numEntities, phaseName );
            }
            ...
            vals1 = SaveToArray( modeShape, numDofs, numEntities, writeName );
            vals2 = SaveToArray( ampl, numDofs, numEntities, amplName );
          } // harmonic data as mode shape
        } // if isComplex

        // save values to grid
        if( entType == H5NACS::NODE) { 
...
vtkDoubleArray* vtkNACSReader::SaveToArray( const std::vector<double>& vals,
...
//----------------------------------------------------------------------------
void vtkNACSReader::ReadCells(vtkMultiBlockDataSet *output)
{

  std::vector<H5NACS::ElemType> types;
  ...
  try {
    ...
    iRegion < n; iRegion++ ) {
      ...
      // Add a result vector containing the NACS element types
      // to the current unstructured grid.
      ...
      for( unsigned int i = 0; i < numRegionElems; i++ ) {
        ...
        unsigned int actElem = regionElems[i];
        ...
        for(unsigned int j = 0, n = connect[actElem-1].size(); j < n; j++)
        {
          ...
          nodeIds[j] = nodeMap_[iRegion][connect[actElem-1][j]-1]-1;
        }
        ...
        if(elemType == VTK_TRIQUADRATIC_HEXAHEDRON) 
        {
          ...
          nodeIds[20] = nodeMap_[iRegion][connect[actElem-1][23]-1]-1;
          nodeIds[21] = nodeMap_[iRegion][connect[actElem-1][21]-1]-1;
          nodeIds[22] = nodeMap_[iRegion][connect[actElem-1][20]-1]-1;
          nodeIds[23] = nodeMap_[iRegion][connect[actElem-1][22]-1]-1;
        }
...
//----------------------------------------------------------------------------
void vtkNACSReader::ReadNodes(vtkMultiBlockDataSet *output)
{
...
VTKCellType  vtkNACSReader::GetCellIdType( H5NACS::ElemType cfsType) {
  VTKCellType type =  VTK_EMPTY_CELL;
  switch( cfsType ) {
    case H5NACS::ET_UNDEF:
      break;
    case H5NACS::ET_POINT:
      type = VTK_VERTEX;
      break;
    case H5NACS::ET_LINE2:
      type = VTK_LINE;
      break;
    case H5NACS::ET_LINE3:
      type = VTK_QUADRATIC_EDGE;
      break;
    case H5NACS::ET_TRIA3:
      type = VTK_TRIANGLE;
      break;
    case H5NACS::ET_TRIA6:
      type = VTK_QUADRATIC_TRIANGLE;
      break;
    case H5NACS::ET_QUAD4:
      type = VTK_QUAD;
      break;
    case H5NACS::ET_QUAD8:
      type = VTK_QUADRATIC_QUAD;
      break;
    case H5NACS::ET_QUAD9:
      type = VTK_BIQUADRATIC_QUAD;
      break;
    case H5NACS::ET_TET4:
      type = VTK_TETRA;
      break;
    case H5NACS::ET_TET10:
      type = VTK_QUADRATIC_TETRA;
      break;
    case H5NACS::ET_HEXA8:
      type = VTK_HEXAHEDRON;
      break;
    case H5NACS::ET_HEXA20:
      type = VTK_QUADRATIC_HEXAHEDRON;
      break;
    case H5NACS::ET_HEXA27:
      type = VTK_TRIQUADRATIC_HEXAHEDRON;
      break;
    case H5NACS::ET_PYRA5:
      type = VTK_PYRAMID;
      break;
    case H5NACS::ET_PYRA13:
      type = VTK_QUADRATIC_PYRAMID;
      break;
    case H5NACS::ET_WEDGE6:
      type = VTK_WEDGE;
      break;
    case H5NACS::ET_WEDGE15:
...
void vtkNACSReader::PrettyPrintFreq(double freq, std::string& val) {
...
void vtkNACSReader::PrettyPrintTime(double time, std::string& val) {
