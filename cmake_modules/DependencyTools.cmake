...
macro(dump_depencency_variables)
  ...
  cmake_print_variables(PACKAGE_ARTIFACT) # a delivered file for header only packages, see create_unpack_precompiled()
  ...
  cmake_print_variables(DEPS_PREFIX)  # usually "${CMAKE_BINARY_DIR}/cfsdeps/${PACKAGE_NAME}"
  cmake_print_variables(DEPS_SOURCE)  # usually "${DEPS_PREFIX}/src/${PACKAGE_NAME}"
...
# alternatively use generate_packing_script_install_dir()
...
# unpack a precompiled package at build time. Header only packages need UNPACK_ARTIFACTS set
# Does not use ExternalProject. Files get current time. 
macro(create_unpack_precompiled)
  assert_set(PACKAGE_NAME)
  assert_set(PRECOMPILED_PCKG_FILE)
  assert_set(DEPS_PREFIX)

  # the artifacts become the output of the custom command, hence they need to be files which
  # the archive really delivers. Either PACKAGE_LIBRARY PACKAGE_ARTIFACT (header only)
  if(PACKAGE_ARTIFACT)
    set(UNPACK_ARTIFACTS "${PACKAGE_ARTIFACT}")
  else()
    # PACKAGE_LIBRARY may also contain plain linker flags (vtk appends "dl") -> filter for files
    unset(UNPACK_ARTIFACTS)
    foreach(ITEM ${PACKAGE_LIBRARY})
      if(IS_ABSOLUTE "${ITEM}")
        list(APPEND UNPACK_ARTIFACTS "${ITEM}")
      endif()
    endforeach()
    if(NOT UNPACK_ARTIFACTS)
      message(FATAL_ERROR "${PACKAGE_NAME}: set PACKAGE_ARTIFACT, as PACKAGE_LIBRARY holds no file")
    endif()
  endif()

  # the extraction itself needs file(ARCHIVE_EXTRACT), which is only available in script mode
  set(UNPACK_SCRIPT "${DEPS_PREFIX}/${PACKAGE_NAME}-DepsUnpackPrecompiled.cmake")
  configure_file("${CMAKE_SOURCE_DIR}/cmake_modules/DepsUnpackPrecompiled.cmake.in" "${UNPACK_SCRIPT}" @ONLY)

  add_custom_command(OUTPUT ${UNPACK_ARTIFACTS}
    COMMAND ${CMAKE_COMMAND} -P "${UNPACK_SCRIPT}"
    DEPENDS "${PRECOMPILED_PCKG_FILE}" "${UNPACK_SCRIPT}"
    COMMENT "unpack precompiled ${PACKAGE_NAME}"
    VERBATIM)

  # the target has the same name as in the build case, such that add_dependencies(cgns hdf5) works
  add_custom_target(${PACKAGE_NAME} DEPENDS ${UNPACK_ARTIFACTS})
endmacro()
