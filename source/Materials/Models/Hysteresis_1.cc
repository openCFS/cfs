...
//          std::cout << "hystValAtXSat: " << hystValAtXSat << std::endl;
    ...
    if(INV_params_.inversionMethod == 5){
      ...
      for(UInt i = 0; i < maxIter; i++){
        ...
        mu.Mult(xVal,deltaX);
        deltaX.Add(1.0,hystVal);
        deltaX.Add(-1.0,yVal);

        if(deltaX.NormL2() < INV_params_.tolH){
          return xVal;
        }
        xVal.Add(-1.0/muFP,deltaX);
      }

      WARN("FP inversion not successful!");
      std::cout << "Final xVal: " << xVal << std::endl;
      std::cout << "Final deltaX.NormL2(): " << deltaX.NormL2() << std::endl;
      return xVal;
