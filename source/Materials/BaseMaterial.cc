...
namespace CoupledField
{
  ...
  void BaseMaterial::matTypeNotAllowed(MaterialType matType, const string &dim) const
  {
    ...
    EXCEPTION("Material type (" << dim << ") " << help << " is not available for " << MaterialClassEnum.ToString(class_)
                                << " Database");
  }
                                          ...
                                          const Vector<T> &origMatTensor)
  {
    ...
    // sigma_11
    ret[0] = (r[0][0] * r[0][0]) * s[0] + (r[0][1] * r[0][1]) * s[1] + (r[0][2] * r[0][2]) * s[2] + 2. * (r[0][0] * r[0][1] * s[5] + r[0][0] * r[0][2] * s[4] + r[0][1] * r[0][2] * s[3]);

    // sigma_22
    ret[1] = (r[1][0] * r[1][0]) * s[0] + (r[1][1] * r[1][1]) * s[1] + (r[1][2] * r[1][2]) * s[2] + 2. * (r[1][0] * r[1][1] * s[5] + r[1][0] * r[1][2] * s[4] + r[1][1] * r[1][2] * s[3]);

    // sigma_33
    ret[2] = (r[2][0] * r[2][0]) * s[0] + (r[2][1] * r[2][1]) * s[1] + (r[2][2] * r[2][2]) * s[2] + 2. * (r[2][0] * r[2][1] * s[5] + r[2][0] * r[2][2] * s[4] + r[2][1] * r[2][2] * s[3]);

    // sigma_23
    ret[3] = r[1][0] * r[2][0] * s[0] + r[1][1] * r[2][1] * s[1] + r[1][2] * r[2][2] * s[2] + (r[1][0] * r[2][1] + r[1][1] * r[2][0]) * s[5] + (r[1][1] * r[2][2] + r[1][2] * r[2][1]) * s[3] + (r[1][0] * r[2][2] + r[1][2] * r[2][0]) * s[4];

    // sigma_13
    ret[4] = r[0][0] * r[2][0] * s[0] + r[0][1] * r[2][1] * s[1] + r[0][2] * r[2][2] * s[2] + (r[0][0] * r[2][1] + r[0][1] * r[2][0]) * s[5] + (r[0][1] * r[2][2] + r[0][2] * r[2][1]) * s[3] + (r[0][0] * r[2][2] + r[0][2] * r[2][0]) * s[4];

    // sigma_12
    ret[5] = r[0][0] * r[1][0] * s[0] + r[0][1] * r[1][1] * s[1] + r[0][2] * r[1][2] * s[2] + (r[0][0] * r[1][1] + r[0][1] * r[1][0]) * s[5] + (r[0][1] * r[1][2] + r[0][2] * r[1][1]) * s[3] + (r[0][0] * r[1][2] + r[0][2] * r[1][0]) * s[4];
                                            ...
                                            bool adjustDamping, bool isHarmonic)
  {
    ...
    if (IsSet(RAYLEIGH_ALPHA) && IsSet(RAYLEIGH_BETA) && IsSet(RAYLEIGH_FREQUENCY))
    {
      Double a, b;
