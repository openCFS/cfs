...
//		std::cerr << "stdMat ("<< rowIndList_[i] << "," <<colIndList1_[j] << ")	";
//		std::cerr << "elemMat("<< rowInd   << ","<< colInd << ")	";
//		std::cerr << elemMat[ rowInd * length2 + colInd ] << std::endl;
  ...
  //   SetCounterPartOnly
  // **********************
  void EntryManipulatorDouble::SetCounterPartOnly(FEMatrixType matrixID,
	...
	if (setTransposeInt == true) {
...
#ifdef DEBUG_ASSEMBLE
		(*debug) << "\n free <-> free matrix (tranpose counterpart):\n";
#endif
		...
		for (UInt i = 0; i < rowIndList_.size(); i += 2) {
			...
			for (UInt j = 0; j < colIndList1_.size(); j += 2) {
				...
				// this is going to insert the transpose of the local
		...
		// check this part, maybe is not working correctly
#ifdef DEBUG_ASSEMBLE
		(*debug) << "\n free <-> fixed matrix:\n";
#endif
		for (UInt i = 0; i < rowIndList_.size(); i += 2) {
	...
	//          the counterpart of the global matrix
		...
		for (UInt j = 0; j < colIndList1_.size(); j += 2) {
			...
			for (UInt i = 0; i < rowIndList_.size(); i += 2) {
				...
				stdMat->AddToMatrixEntry(colIndList1_[j], rowIndList_[i],
						elemMat[ colInd * length1 + rowInd]);

//				std::cerr << "D("<< colIndList1_[j]<< ","<<rowIndList_[i]
//						<< ")	";
//				std::cerr << "elemMat("<< colInd << ","<< rowInd << ")	";
//				std::cerr << elemMat[ colInd * length1 + rowInd]<< std::endl;
		...
		for (UInt j = 0; j < colIndList2_.size(); j += 2) {
			...
			for (UInt i = 0; i < rowIndList_.size(); i += 2) {
				...
				<< rowIndList_[i] << ") += "
				<< elemMat[ colInd * length1 + rowInd ] << std::endl;
#endif
				idbcHandler->AddWeightFixedToFree(matrixID, pdeID1, pdeID2,
						colIndList2_[j], rowIndList_[i],
						elemMat[ colInd * length1
						+ rowInd ]);
			}
		}
	}
  ...
  //   SetNodeRHS (Double)
  // ***********************
  void EntryManipulatorDouble::SetNodeRHS( SparseVector *rhs, Double val,
                                           Integer node ) {
  ...
  //   SetNodeRHS (Complex)
  // ***********************
  void EntryManipulatorDouble::SetNodeRHS( SparseVector *rhs, Complex val,
                                           Integer node ) {
  ...
  //   SetVectorEntry
  // ******************
  void EntryManipulatorDouble::SetVectorEntry( SparseVector *vec,
                                               UInt index,
                                               Double &newVal ) {
  ...
  //   InitSol
  // ***********
  void EntryManipulatorDouble::InitSol( SparseVector *sol,
                                        const Double *newSol ) {


    // For the standard vector class we use fast access
    if ( Vector<Double> *mySol = dynamic_cast<Vector<Double>*>(sol) ) {
      for ( UInt i = 1; i <= sol->GetSize(); i++ ) {
        (*mySol)[i] = (newSol-1)[i];
      }
    }
  ...
  //   InitRHS
  // ***********
  void EntryManipulatorDouble::InitRHS( SparseVector *rhs,
                                        const Double *newRHS ) {
  ...
  //   AdaptRHSForIDBC
  // *******************
  void EntryManipulatorDouble::AdaptRHSForIDBC( SparseVector &rhs,
                                                UInt *dirichletEQN,
  ...
  void EntryManipulatorDouble::GetSolutionVal( SparseVector *sol, Double* &ptSol,
  ...
  void
  EntryManipulatorDouble::GetSolutionVal( SparseVector *sol, Complex* &ptSol,
                                          const PdeIdType identifierPDE ) {
  ...
  void EntryManipulatorDouble::GetRHSVal( SparseVector *rhs, Double* &ptRhs,
                                          const PdeIdType identifierPDE ) {
