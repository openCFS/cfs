...
namespace OLAS {
  ...
  public:
    ...
    //! \param blockSize   blocksize of entries of sub-matrix
    ...
    void SetSubMatrix( Integer i, Integer j, MatrixEntryType eType,
                       MatrixStorageType sType, Integer blockSize,
                       Integer nrows, Integer ncols, Integer nnz );
    ...
    //! an entry of a sub-matrix corresponds. A sub-matrix using scalar
    //! entries e.g. has a block size of 1, while a sub-matrix with 4x4
    //! matrices as entries has a block size of 4.
    ...
    //! Print the matrix to the output stream os
    void Print( std::ostream &os ) const {
      Warning( "Print not implemented for SBM_Matrix", __FILE__, __LINE__ );
    };
    ...
    const StdMatrix* GetPointer( Integer i, Integer j ) const {
      return (subMat_[ComputeIndex(i,j)]);
    };
    ...
    //! The method returns a pointer to a sub-matrix. The access follows the
    //! one-based indexing convention of OLAS.
    StdMatrix* GetPointer( Integer i, Integer j ) {
      return (subMat_[ComputeIndex(i,j)]);
    };
  ...
  private:
    ...
    //! Attrribute storing the block size of the matrix

    //! Currently the %SBM_Matrix class only allows for storing sub-matrices
    //! that share a common block size. This attribute stores the block size
    //! of the first sub-matrix that is set and uses this as the over-all
    //! block size of the %SBM_Matrix object.
    UInt myBlockSize_;
