...
template<typename T>
void StdMatrix::HarwellBoeing<T>::Fill(const T* values, const Integer* ints, UInt length, std::vector<std::string>& out)
{
    // No label                                                                No key  
    //             77             4            21            41             7
    // RSA                       26            26           163
...
void StdMatrix::HarwellBoeing<T>::Export(const std::string& file, const BaseVector& rhs)
{
   ...
   switch(matrix_->GetStorageType())
   {
      case SPARSE_SYM:
        {
           ...
           Fill(NULL, scrs->GetRowPointer()+1,scrs->GetNrows()+1, cols);  // include tail
           
           // Nnz is the total number = Diagonal + 2 * triagonals. But one is skipped!
           elements= scrs->GetNnz() - ((scrs->GetNnz() - scrs->GetNrows())/2);
     ...
     case SPARSE_NONSYM:
        { 
           ...
           NewArray(col_ptr, int, crs->GetNcols() + 1);
           NewArray(row_ptr, int, elements);
           NewArray(val_ptr, T, elements);

           crs->Transpose(col_ptr, row_ptr, val_ptr);
 
           Fill(NULL, col_ptr+1, crs->GetNcols()+1, cols);  // include tail
   ...
   Fill(vector.GetPointer()+1,NULL,vector.GetDataSize(), rhs_data);
   ...
   out << code + "           ";
   out.width(14);
   out << matrix_->GetNrows();        
   out.width(14);
   out << matrix_->GetNcols();
   out.width(14);
   out << elements; 
   out <<  std::endl;
   ...
   for(unsigned int i = 0; i < cols.size(); i++) out << cols[i];        
   for(unsigned int i = 0; i < rows.size(); i++) out << rows[i];
   for(unsigned int i = 0; i < mat_data.size(); i++) out << mat_data[i];
   for(unsigned int i = 0; i < rhs_data.size(); i++) out << rhs_data[i];      
