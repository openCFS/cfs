...
namespace CoupledField {
  ...
  // ***************
  template<typename T>
  COCRSolver<T>::COCRSolver( PtrParamNode solverNode, PtrParamNode olasInfo ) {
    xml_      = solverNode;
  ...
  void COCRSolver<T>::Solve( const BaseMatrix &sysMat, const BaseVector &rhs, BaseVector &sol ) {
    ...
    Double resNorm   = 0;
    Double threshold = ComputeThreshold( eps_, rhs, *r_, resNorm, false );
    // Hard floor for breakdown detection on rho = (qtld, q)
    const Double breakdownEps = 1e-30;

    LOG_DBG(cocrsolver) << "COCR initial ||r||_2 = " << resNorm << ", threshold = " << threshold;
    ...
    if ( resNorm <= threshold ) {
      ...
      // q_0 = A p_0
      sysMat.Mult( *p_, *q_ );
      // z must be zero here; Solve no longer re-runs Setup
      z_->Init();
      // z_0 = p_0
      z_->Add( *p_ );
      ...
      while ( niter < maxIter_ ) {
        ...
        //! A breekdown free version of COCR exists and could be implemented in the future, see
        //! Zhong H-X, Gu X-M, Zhang S-L,
        //! "A Breakdown-Free Block COCG Method for Complex Symmetric Linear Systems with Multiple Right-Hand Sides.",
        //! Symmetry. 2019; 11(10):1302. https://doi.org/10.3390/
        if ( Abs( rho ) < breakdownEps ) {
          WARN( "COCRSolver: break down at iteration " << niter << " (|rho| = " << Abs( rho ) << "). Returning best solution so far." );
          break;
        }
