...
#define BOOST_DECLARE_LOG(log_name) \
    struct log_name ## _log_class { typedef enum { is_compile_time = 0, is_enabled = 1 } dummy; }; \
...
#define BOOST_DECLARE_LOG_COMPILE_TIME(log_name, compile_constant) \
    struct log_name ## _log_class { typedef enum { is_compile_time = 1, is_enabled = ((compile_constant) != 0) } dummy; }; \
    ::boost::logging::logger & log_name();
