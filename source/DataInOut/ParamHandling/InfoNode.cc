...
bool InfoNode::IsGoodAttribute(const std::string& string) const
{
  if(string.size() > 30) return false;
  if(string.find('\"') != string::npos) return false;
  return true;
}
...
void InfoNode::SetValue(const std::string& string)
{
  ...
  if(name_ == WARNING)
  {
    cerr << fg_magenta << "Warning: /" << fg_reset;
    ...
    cerr << ": " << fg_magenta << "\"" << fg_reset << string << fg_magenta << "\"" << fg_reset << endl;
  }
}
