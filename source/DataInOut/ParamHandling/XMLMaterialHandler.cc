...
namespace CoupledField
{
  ...
  void XMLMaterialHandler::ReadAcoustic(BaseMaterial *material, PtrParamNode acou)
  {
    ...
    // read density
    if (acou->Has("density"))
    { // check if PtrParamNode acou has <density> tag
      ...
      if (dens->Has("rationalFuncApproxInverse"))
      {
        ...
        {
          // TODO
          std::cout << " rational function  provided: InvDensity " << std::endl
                    << std::endl;
    ...
    if (acou->Has("compressionModulus"))
    {
      ...
      if (blk->Has("rationalFuncApproxInverse"))
      {
        // TODO
        std::cout << " rational function  provided: InvBlkMod " << std::endl
                  << std::endl;
  ...
  void XMLMaterialHandler::ReadHystOperator(BaseMaterial *material, PtrParamNode operatorNode, bool setStrains, bool isMagnetic)
  {
    ...
    if (isPreisachType)
    {
      if (pWeight != NULL)
      {
        // Read in weights
        int dim = -1;
        if (pWeight->Has("dim_weights"))
          dim = pWeight->Get("dim_weights")->As<Integer>();
