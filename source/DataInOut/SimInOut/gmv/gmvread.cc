...
namespace CoupledField {
  ...
  // But I cannot tolerate such behavior in a library.
  // In combination with the exceptions in the gmvrdmemerr(), ioerrtst(),
  // and gmvrdmemerr2() this is the foundation of my error handling concept.

#define FREE(a) { if (a) free(a); gmv_mem_alloc_set.erase(a); a = NULL; }
#define MALLOC(a,num,type) { a = (type*) malloc((num)*sizeof(type)); if(a) gmv_mem_alloc_set.insert(a); }
#define CALLOC(a,num,type) { a = (type*) calloc(num,sizeof(type)); if(a) gmv_mem_alloc_set.insert(a); }
#define REALLOC(a,num,type) { type* save = a; a = (type*) realloc(a,(num)*sizeof(type)); if(a) {gmv_mem_alloc_set.erase(save); gmv_mem_alloc_set.insert(a);} else {if(num==0) gmv_mem_alloc_set.erase(a);} }

#define FCLOSE(file) { if(file != NULL) {fclose(file); file = NULL;} }
...
#define IECXI8R8 8
  ...
  void swapbytes(void *from, int size, int nitems),
    ...
    readvectors(FILE *gmvin, int ftype),
    gmvrdmemerr(), ioerrtst(FILE *gmvin), endfromfile();
  static FILE *gmvin, *gmvin_sav;

  std::vector<CellType> gmv_cell_types;
  std::string gmv_base_dir;
  std::set<void*> gmv_mem_alloc_set;
  ...
  void gmvread_data()
  {
    ...
    if (readkeyword == 1)
    {
      ...
      else if (strncmp(keyword,"vectors",7) == 0) curr_keyword = VECTOR;
      ...
      case(PROBTIME):
        ...
        MALLOC(gmv_data.doubledata1, 1, double);
  ...
  int rdints(int iarray[], int nvals, FILE* gmvin)
  {
    /*                                                  */
  ...
  long rdlongs(long iarray[], long nvals, FILE* gmvin)
  {
    /*                                                  */
    /*  Read an integer array from an ASCII text file.  */
    /*                                                  */
  ...
  long rdfloats(double farray[], long nvals, FILE* gmvin)
  {
    /*                                                  */
    /*  Read an integer array from an ASCII text file.  */
    /*                                                  */
  ...
  void checkfromfile()
  {
    char c, *charptr, tmpbuf[200], stringbuf[100], buffer[4096];
    int i, ierr, fkeyword;
    ...
    if (fromfileskip == 1 && fromfileflag == 0)
    {
      ...
      MALLOC(gmv_data.chardata1, i, char);
  ...
  void readnodes(FILE* gmvin, int ftype)
  {
    ...
    if (lnodes == -3)
    {
      ...
      MALLOC(lxic, 3, double);
      MALLOC(lyic, 3, double);
      if (ftype != ASCII)
      {
        if (ftype == IEEEI4R8 || ftype == IEEEI8R8)
        {
    ...
    if (lstructuredflag == 0 || lstructuredflag == 2)
    {
      ...
      MALLOC(lxic, lnodes, double);
      MALLOC(lyic, lnodes, double);
      MALLOC(lzic, lnodes, double);
      ...
      if (ftype != ASCII)
      {
        if (ftype == IEEEI4R8 || ftype == IEEEI8R8)
        {
          ...
          MALLOC(tmpdouble, 3*lnodes, double);
          if (tmpdouble == NULL)
          {
            gmvrdmemerr();
            return;
        ...
        else
        {
          ...
          MALLOC(tmpfloat, 3*lnodes, float);
          if (tmpfloat == NULL)
          {
            gmvrdmemerr();
            return;
      ...
      if (ftype == ASCII)
      {
        //tmpdouble = (double *)calloc((3*lnodes), sizeof(double));
        CALLOC(tmpdouble, 3*lnodes, double);
        if (tmpdouble == NULL)
        {
          gmvrdmemerr();
          return;
        ...
        long dim;
        long n = rdfloats(tmpdouble,lnodes*3,gmvin);
        long nsav = n;
        for(dim=1; dim<=3; dim++)
        {
          n -= lnodes;
    ...
    if (lstructuredflag == 1)
    {
      ...
      MALLOC(lxic, lnxv, double);
      MALLOC(lyic, lnyv, double);
      MALLOC(lzic, lnzv, double);
  ...
  void readcells(FILE* gmvin, int ftype)
  {
    ...
    if (amrflag_in)
    {
      ...
      if (ftype != ASCII)
      {
        if (ftype == IEEEI4R4 || ftype == IEEEI4R8)
        {
          ...
          MALLOC(cellnodenos, lncells, int);
          if (cellnodenos == NULL)
          {
            gmvrdmemerr();
            return;
    ...
    if (strncmp(keyword,"general",7) == 0)
    {
      ...
      MALLOC(lcellnodenos, totverts, long);
      ...
      if (ftype != ASCII)
      {
        if (ftype == IEEEI4R4 || ftype == IEEEI4R8)
        {
          ...
          MALLOC(cellnodenos, totverts, int);
          if (cellnodenos == NULL)
          {
            gmvrdmemerr();
            return;
  ...
  void readfaces(FILE* gmvin, int ftype)
  {
    ...
    else
    {
      ...
      else 
      {
        ...
        MALLOC(tmpvertsin, nverts+2, int);
        if (tmpvertsin == NULL)
        {
          gmvrdmemerr();
          return;
  ...
  void readvfaces(FILE* gmvin, int ftype)
  {
    ...
    MALLOC(gmv_data.longdata2, 4, long);
  ...
  void readmats(FILE* gmvin, int ftype)
  {
    ...
    MALLOC(matnames, lmmats*33, char);
    if (matnames == NULL)
    {
      gmvrdmemerr();
      return;
    ...
    MALLOC(gmv_data.longdata1, lnmatin, long);
  ...
  void readvels(FILE* gmvin, int ftype)
  {
    ...
    MALLOC(uin, nvelin, double);
    MALLOC(vin, nvelin, double);
    MALLOC(win, nvelin, double);
  ...
  void readvectors(FILE* gmvin, int ftype)
  {
    /*                               */
    ...
    if (ftype != ASCII) binread(&cnameflag,intsize,INT,(long)1,gmvin);
    if (ftype == ASCII) 
    {
      char c;
      std::vector<char> buf;
      ...
      sscanf(&buf[0],"%d",&cnameflag);

      cnameflag = cnameflag == 1 ? 1 : 0;
    }
    ioerrtst(gmvin);
    ...
    if(cnameflag != 0)
    {
      for(i=0; i<ncomps; i++)
      {
        if (ftype != ASCII)
        {
          binread(compname,charsize,CHAR,(long)charsize_in,gmvin);
    ...
    MALLOC(vectin, ncomps*nvectin, double);
    ...
    if (ftype != ASCII)
    {
      if (ftype == IEEEI4R8 || ftype == IEEEI8R8)
      {
        binread(vectin,doublesize,DOUBLE,(long)ncomps*nvectin,gmvin);
        ioerrtst(gmvin);
      }
      else
      {
        MALLOC(tmpfloat, ncomps*nvectin, float);
        ...
        binread(tmpfloat,floatsize,FLOAT,(long)ncomps*nvectin,gmvin);
        ioerrtst(gmvin);
        for (i = 0; i < ncomps*nvectin; i++) vectin[i] = tmpfloat[i];
        FREE(tmpfloat);
      }
    }
    if (ftype == ASCII)
    {
      rdfloats(vectin,(long)ncomps*nvectin,gmvin);
    }
  ...
  void readflags(FILE* gmvin, int ftype)
  {
    ...
    MALLOC(gmv_data.longdata1, nflagin, long);
    if (gmv_data.longdata1 == NULL)
    {
      gmvrdmemerr();
      return;
  ...
  void readpolygons(FILE* gmvin, int ftype)
  {
    ...
    /*  Check for endpoly.  */
    if ( (strncmp(varname,"endpoly",7) == 0) ||
         (strncmp(varname,"endp",4) == 0) ||
         (strncmp(varname,"end",3) == 0)
      )
    {
    ...
    MALLOC(vertsin, 3*nvertsin, double);
    if (vertsin == NULL)
    {
      gmvrdmemerr();
      return;
    ...
    if (ftype != ASCII)
    {
      ...
      else
      {
        ...
        MALLOC(tmpfloat, 3*nvertsin, float);
        if (tmpfloat == NULL)
        {
          gmvrdmemerr();
          return;
    ...
    MALLOC(gmv_data.doubledata1, nvertsin, double);
    ...
    MALLOC(gmv_data.doubledata2, nvertsin, double);
    ...
    MALLOC(gmv_data.doubledata3, nvertsin, double);
  ...
  void readtracers(FILE* gmvin, int ftype)
  {
    ...
    if (readkeyword == 1)
    {
      ...
      if (numtracers > 0)
      {
        ...
        MALLOC(lxtr, numtracers, double);
        MALLOC(lytr, numtracers, double);
        MALLOC(lztr, numtracers, double);
        ...
        if (ftype != ASCII)
        {
          ...
          else
          {
            ...
            MALLOC(tmpfloat, numtracers, float);
    ...
    if (numtracers > 0)
    {
      ...
      MALLOC(lfieldtr, numtracers, double);
      if (lfieldtr == NULL)
      {
        gmvrdmemerr();
        return;
  ...
  void readtracerids(FILE* gmvin, int ftype)
  {
    ...
    if (numtracers > 0)
    {
      ...
      MALLOC(ltracerids, numtracers, long);
      if (ltracerids == NULL)
      {
        gmvrdmemerr();
        return;
  ...
  void readunits(FILE* gmvin, int ftype)
  {
        ...
        strncmp(unittype,"velocity",8) == 0)
    {
      ...
      MALLOC(gmv_data.chardata1, 20, char);
      ...
      MALLOC(gmv_data.chardata2, 20, char);
        ...
        strncmp(unittype,"faces",5) == 0)
    {
      ...
      MALLOC(fldstr, numunits*33, char);
      MALLOC(unitstr, numunits*33, char);
  ...
  void readsurfmats(FILE* gmvin, int ftype)
  {
    ...
    MALLOC(gmv_data.longdata1, numsurf, long);
    if (gmv_data.longdata1 == NULL)
    {
      gmvrdmemerr();
      return;
  ...
  void readsurfvel(FILE* gmvin, int ftype)
  {
    ...
    MALLOC(uin, numsurf, double);
    MALLOC(vin, numsurf, double);
    MALLOC(win, numsurf, double);
  ...
  void readgroups(FILE* gmvin, int ftype)
  {
    ...
    MALLOC(gmv_data.longdata1, ngroupin, long);
  ...
  void readsubvars(FILE* gmvin, int ftype)
  {
    ...
    MALLOC(subvarid, nsubvarin, int);
    MALLOC(subvarin, nsubvarin, double);
    ...
    if (ftype != ASCII)
    {
      ...
      else
      {
        ...
        MALLOC(tmpfloat, nsubvarin, float);
        if (tmpfloat == NULL)
        {
          gmvrdmemerr();
          return;
    ...
    MALLOC(gmv_data.longdata1, nsubvarin, long);
    if (gmv_data.longdata1 == NULL)
    {
      gmvrdmemerr();
      return;
  ...
  void readghosts(FILE* gmvin, int ftype)
  {
    ...
    MALLOC(gmv_data.longdata1, nghostin, long);
    if (gmv_data.longdata1 == NULL)
    {
      gmvrdmemerr();
      return;
  ...
  int binread(void* ptr, int size, int type, long nitems, FILE* stream)
  {
    ...
    case SHORT:
      ...
      MALLOC(shortbuf, size/sizeof(short), short);
      shortptr = (short *)ptr;
    ...
    case INT:
      ...
      MALLOC(intbuf, size/sizeof(int), int);
    ...
    case FLOAT:
      ...
      MALLOC(floatbuf, size/sizeof(float), float);
    ...
    case DOUBLE:
      ...
      MALLOC(doublebuf, size/sizeof(double), double);
    ...
    default:
      std::cerr << "Error: Cannot match input datatype.");
    gmv_data.keyword = GMVERROR;
    return;
  }
  ...
  if(tierr != 0)
  {
    std::cerr << "Error: Cannot convert IEEE data to CRAY");
  gmv_data.keyword = GMVERROR;
  return;
}
...
void gmvread_mesh()
{
  ...
  if (gmv_data.keyword == NODES)
  {
    ...
    if (gmv_data.datatype != AMR)
    {
      ...
      MALLOC(gmv_meshdata.x, nn, double);
      //gmv_meshdata.y = (double *)malloc(nn*sizeof(double));
      MALLOC(gmv_meshdata.y, nn, double);
      //gmv_meshdata.z = (double *)malloc(nn*sizeof(double));
      MALLOC(gmv_meshdata.z, nn, double);
        ...
        gmv_data.datatype == LOGICALLY_STRUCT)
    {
      ...
      if (gmv_data.datatype == STRUCT)
      {
        ...
        MALLOC(xin, nxv, double);
        MALLOC(yin, nyv, double);
        MALLOC(zin, nzv, double);
    ...
    if (gmv_data.datatype == AMR)
    {
      ...
      MALLOC(gmv_meshdata.x, 2, double);
      MALLOC(gmv_meshdata.y, 2, double);
      MALLOC(gmv_meshdata.z, 2, double);
...
void rdcells(int nodetype_in)
{
  ...
  if (gmv_data.datatype == AMR)
  {
    ...
    MALLOC(gmv_meshdata.celltoface, nc, long);
  ...
  MALLOC(celltoface, nc, long);
  MALLOC(cell_faces, nc*nfa, long);
  ...
  if (gmv_meshdata.intype == CELLS)
  {
    ...
    MALLOC(cellnnode, nc, long);
    //cellnodes = (long *)malloc(1*sizeof(long));
    MALLOC(cellnodes, 1, long);
    ...
    if (gmv_data.datatype == ENDKEYWORD)
    {
      ...
      else
      {
        ...
        if (totcellnodes > 0)
        {
          ...
          REALLOC(cellnodes, totcellnodes, long);
...
void regcell(long icell, long nc)
{
  ...
  if (totcellnodes + ncnodes > cellnodes_alloc)
  {
    ...
    REALLOC(cellnodes, cellnodes_alloc, long);
  }
  for (i = 0; i < ncnodes; i++)
  ...
  gmv_cell_types.push_back((CellType)icelltype);
...
void fillmeshdata(long nc)
{
  ...
  REALLOC(cell_faces, totfaces+1, long);
  ...
  REALLOC(facetoverts, nfacesin+1, long);
  ...
  REALLOC(faceverts, nvertsin, long);
...
void rdfaces()
{
  ...
  MALLOC(celltoface, nc, long);
  MALLOC(facetoverts, nfacesin, long);
  MALLOC(faceverts, nfacesin*8, long);
  faceverts_alloc = nfacesin*8;
  MALLOC(facecell1, nfacesin, long);
  MALLOC(facecell2, nfacesin, long);
...
void rdvfaces(long nc)
{
  ...
  MALLOC(facecell1, nfacesin, long);
  MALLOC(facecell2, nfacesin, long);
  MALLOC(facepe, nfacesin, long);
  MALLOC(oppface, nfacesin, long);
  MALLOC(oppfacepe, nfacesin, long);
...
void rdxfaces()
{
  ...
  MALLOC(facetoverts, nfacesin, long);
  MALLOC(faceverts, totverts, long);
  MALLOC(facecell1, nfacesin, long);
  MALLOC(facecell2, nfacesin, long);
  MALLOC(facepe, nfacesin, long);
  MALLOC(oppface, nfacesin, long);
...
void fillcellinfo(long nc, long *facecell1, long *facecell2)
{
  ...
  MALLOC(cell_faces, totfaces, long);
  ...
  REALLOC(facecell1, nfacesin, long);
  REALLOC(facecell2, nfacesin, long);
...
void struct2vface()
{
  ...
  MALLOC(gmv_meshdata.facecell1, nf, long);
  MALLOC(gmv_meshdata.facecell2, nf, long);
  ...
  MALLOC(gmv_meshdata.vfacepe, nf, long);
  MALLOC(gmv_meshdata.vfaceoppface, nf, long);
  MALLOC(gmv_meshdata.vfaceoppfacepe, nf, long);
...
void struct2face()
{
  ...
  MALLOC(gmv_meshdata.cellfaces, totf+1, long);
  if (gmv_meshdata.cellfaces == NULL) gmvrdmemerr2();
...
// already declared in line 61
// static int ftype;

// already declared in line 63
// static unsigned wordbuf;
...
void readrays(FILE* gmvrayin, int ftype)
{
  ...
  for (iray = 0; iray < lrays; iray++)
  {
    ...
    MALLOC(x, npts, double);
    MALLOC(y, npts, double);
    ...
    if (ftype != ASCII)
    {
      if (ftype == IEEEI4R8 || ftype == IEEEI8R8)
      {
        ...
        MALLOC(tmpdouble, 3*npts, double);
        if (tmpdouble == NULL)
        {
          gmvrayrdmemerr();
          return;
