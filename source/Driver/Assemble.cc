...
namespace CoupledField
{
  ...
  void Assemble::AddBiLinearForm(BiLinFormContext *biLinContext)
  {
    ...
    if (mappedFEType != NOTYPE)
    {
      ...
      if (!found)
      {
        pair = std::pair<shared_ptr<EntityList>, shared_ptr<EntityList>>(biLinContext->GetFirstEntities(), biLinContext->GetSecondEntities());
      }
...
#pragma omp parallel num_threads(CFS_NUM_THREADS)
                                           ...
                                           const StdVector<Double> &multHarmFreqVec)
  {
    ...
    auto ComputeIndex = [size](UInt a, UInt b)
    { return (size)*a + b; };
...
#pragma omp parallel num_threads(CFS_NUM_THREADS)
    ...
    for (it = allBiLinForms_.begin(); it != allBiLinForms_.end(); it++)
    {
      ...
      if (context.GetFirstEntities()->GetType() == EntityList::ELEM_LIST)
      {
        shared_ptr<ElemList> list =
            boost::dynamic_pointer_cast<ElemList, EntityList>(context.GetFirstEntities());
        regionName = list->GetName();
      }
      else if (context.GetFirstEntities()->GetType() == EntityList::SURF_ELEM_LIST)
      {
        shared_ptr<SurfElemList> list =
            boost::dynamic_pointer_cast<SurfElemList, EntityList>(context.GetFirstEntities());
        regionName = list->GetName();
      }
  ...
  void Assemble::CheckNonLinearities(bool setall)
  {
    ...
    for (it = allBiLinForms_.begin(); it != allBiLinForms_.end(); it++)
    {
      ...
      if (actContext.IsNonLin() || analysisType_ == BasePDE::HARMONIC || analysisType_ == BasePDE::MULTIHARMONIC || analysisType_ == BasePDE::INVERSESOURCE || analysisType_ == BasePDE::EIGENFREQUENCY || setall)
      {
  ...
  bool Assemble::IsFEMatSymmetric(BiLinFormContext *actCt)
  {
    ...
    if ((actCt->GetFirstFeFunction().lock() == actCt->GetSecondFeFunction().lock()) && (actCt->GetFirstEntities() == actCt->GetSecondEntities()))
    {
                              ...
                              bool isMultHarmDiag)
  {
    ...
    assert(analysisType_ == BasePDE::MULTIHARMONIC || analysisType_ == BasePDE::HARMONIC || analysisType_ == BasePDE::INVERSESOURCE || analysisType_ == BasePDE::EIGENFREQUENCY || analysisType_ == BasePDE::EIGENVALUE);
