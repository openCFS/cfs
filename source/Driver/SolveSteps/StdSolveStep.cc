...
namespace CoupledField {
      ...
      do {
        ...
        if ( lineSearch_ == "none" || iterationCounter == 1) {
          ...
          if (lineSearch_ == "minResidual") { 
            ...
            EXCEPTION("The specified line search technique is not implemented!");
          }
          // store the new solution
          solVec_ = actSol;
        }
  ...
  **/
  ...
  // ======================================================
  // METHODS FOR 2.5D Harmonic Analysis
  // ======================================================
  void StdSolveStep::StoreSolutionToFeFunctions() {
    algsys_->GetSolutionVal(solVec_);
  }
  ...
  Double StdSolveStep::LineSearchMultHarm(const SBM_Vector& solIncrement, SBM_Vector& actSol,
    ...
    etaLineSearch = etaOpt;
    LOG_DBG2(stdsolvestep) << "------------------------------------------------------------------------------------------------------------" << std::endl;
    LOG_DBG2(stdsolvestep) << "Optimal eta = " << etaOpt << std::endl;
    LOG_DBG2(stdsolvestep) << "Error (L2 norm of actRHS) for optimal eta: " << residualL2Norm << std::endl;
    LOG_DBG2(stdsolvestep) << "Considered Harmonics: " << consideredH_ << std::endl;
    LOG_DBG2(stdsolvestep) << "------------------------------------------------------------------------------------------------------------" << std::endl;
  ...
  void StdSolveStep::SetupGetRidOfZerosActive(){
    ...
    if( setupGetRidOfZerosDone_ ) {
      ...
      if (useCase != "" && hasNCI && getRidOfZerosXML=="auto") {
        ...
        LOG_DBG2(stdsolvestep) << "Zero entities will be removed from the system matrix in each iteration to reduce solver effort because the model contains at least one NCI. Define \"no\" for \"getRidOfZeros\" in \"linearSystems\" explicitly to avoid this.";
