...
MpCCIexch::MpCCIexch(Grid *aptgrid, StdVector<UInt> & mapSD)
{
  ...
  // get parameter node
  myParam_ = param->Get( "MpCCI-flownoise" );


  //Get general specific coupling description for CFS++ side
  myParam_->Get( "meshId", meshId_ );
  myParam_->Get( "partId",partId_ );
  myParam_->Get( "nNodeIds", nNodeIds_ );
  myParam_->Get( "GlobalDim", GlobalDim_ );
  ...
  if( writeSrcFileperNode_ ) {
    ...
    TransientDriver * actDriver =
      dynamic_cast<TransientDriver*>(domain->GetSingleDriver() );
    
    numSteps_ = actDriver->GetNumSteps();
    firstdt_ = actDriver->GetDeltaT();
  ...
  MpCCIexch::MpCCIexch(Grid *aptgrid, StdVector<RegionIdType>  subdoms)
  {
    ...
    //Get general specific coupling description for CFS++ side
    param->Get("mpcciFSI")->Get( "GlobalDim", GlobalDim_ );
  }
...
#define REALTYPE CCI_DOUBLE
  ...
  //  Point ptNodalCoord2D;      
  Point ptNodalCoord3D;      
...
void MpCCIexch::CouplCompPhase(Matrix<Double> & flowdata, Double acttime)
{
  ...
  myParam_->Get("nQuantityIds",nQuantityIds, "MpCCI-flownoise");
  myParam_->Get("nLocalMeshIds",nLocalMeshIds, "MpCCI-flownoise");
  myParam_->Get("remoteCodeId",remoteCodeId, "MpCCI-flownoise");
  myParam_->Get("quantityId1",quantityId1, "MpCCI-flownoise");
  myParam_->Get("quantityId2",quantityId2, "MpCCI-flownoise");
  myParam_->Get("quantityDim1",quantityDim1, "MpCCI-flownoise");
  myParam_->Get("quantityDim2",quantityDim2, "MpCCI-flownoise");
  myParam_->Get("maxnEmptyNodes",maxnEmptyNodes, "MpCCI-flownoise");
