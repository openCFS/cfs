...
//#define DEBUG_SCRS_MATRIX
  ...
  template<typename T>
      SCRS_Matrix<T>::SCRS_Matrix(UInt nr, UInt nc, UInt nnz, UInt* row_ptr, UInt* col_ptr, T* data_ptr) {
        ...
        for ( UInt i = 0; i < this->nnz_; i++ ) {
          if ( col_ptr[i] >= row_ptr[i] ) {
            entriesPerRow[row_ptr[i]]++;
          }
        }
        ...
        for ( UInt j = 0; j < this->nnz_; j++ ) {
          curRow = row_ptr[j];
          if ( curRow <= col_ptr[j] ) {
            data_  [ rowPtr_[curRow] + pos[curRow] ] = data_ptr[j];
            colInd_[ rowPtr_[curRow] + pos[curRow] ] = col_ptr[j];
            pos[ curRow ]++;
          }
        }
