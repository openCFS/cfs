...
//  rIter = upResIds.Begin();
  ...
  for(; rIter != upResIds.End() ; rIter++){
    //if(resultManager_->IsResultVecUpToDate(*rIter)){
	 // std::cout << "actual calculated result:                                                     " << resultManager_->GetResultName(*rIter) << std::endl;
      if(resultManager_->GetExtInfo(*rIter)->isMeshResult){
        ...
        if(resultManager_->GetExtInfo(*rIter)->dType == ExtendedResultInfo::COMPLEX){
          ...
          Vector<Double> & fullVec = GetUpstreamResultVector<Double>(*rIter,aStepIter_->second,eqnVec);   // Get Vector with upstream results
          if (!resultManager_->IsResultVecUpToDate(*rIter)) {
            break;
          }
          //now we loop over the result array and copy the values according to
        ...
        std::set<boost::uuids::uuid> allresMapids = resultManager_->TestMethod();

        std::set<boost::uuids::uuid> resultsID =  resultManager_->GetListofResultsid();  //computed IDs
        std::vector<string> resultsNa =  resultManager_->GetListofResultsName();		//computed Names
        ParamNodeList saveList = params_->Get("saveResults")->GetList("result");		//all variable names


        std::set<uuids::uuid>::iterator aIter = allresMapids.begin();
        for(; aIter != allresMapids.end(); ++aIter){
  		  uuids::uuid aId = *aIter;
  		  //if(aId != resultsID[0]){
  			  //resultManager_->GetResultName(aId);
  		  //}
            ...
            //if(resultName != resultsNa[0]){
            //std::cout << "uncomputed variable: " << resultName << std::endl;
            //}
          }

        //resultManager_->ShowActiveResult();
...
ResultIdList OutputFilter::SetUpstreamResults(){
 ...
 // results = rNode
  ...
  for(UInt aRes = 0; aRes < saveList.GetSize(); ++aRes){
    PtrParamNode rNode = saveList[aRes];
    std::string resultName = rNode->Get("resultName")->As<std::string>();
    std::cout << "Getting variables from variable list of output filter: " << resultName << std::endl;
    ...
    if (hasGlobalStepvalues) {
      resultManager_->SetTimeLine(newId,globalStepValues);                                              //resultMap has all different variables as entries
    }
