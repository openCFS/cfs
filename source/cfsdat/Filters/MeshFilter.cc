...
MeshFilter::MeshFilter(UInt numWorkers, CF::PtrParamNode config, str1::shared_ptr<ResultManager> resMan)
  ...
  if (params_-> Get("type")->As<std::string>() != "AeroacousticSource_LambVector" &&
      ...
      params_-> Get("type")->As<std::string>() != "AeroacousticSource_LighthillSourceTerm" &&
      params_-> Get("type")->As<std::string>() != "AeroacousticSource_LighthillSourceTensor"){
        std::string inRes = params_->Get("singleResult")->Get("inputQuantity")->Get("resultName")->As<std::string>(); //InputQuantity
        std::string outRes = params_->Get("singleResult")->Get("outputQuantity")->Get("resultName")->As<std::string>(); //Output Quantity
        filtResNames.insert(outRes);
        upResNames.insert(inRes);
  }
