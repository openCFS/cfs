...
bool NearestNeighbourInterpolator::UpdateResults(std::set<uuids::uuid>& upResults){
  ...
  // TODO matrix interpolation here
  Matrix& matrix = matrices_[matrixIndex_];
  CF::UInt& numTrgEntities = matrix.numTargets;
  StdVector<CF::UInt>& targetSourceIndex = matrix.targetSourceIndex;
  StdVector<CF::UInt>& targetSource = matrix.targetSource;
  StdVector<CF::Double>& targetSourceFactor = matrix.targetSourceFactor;
  
  NearestNeighbourInterpolation(returnVec, inVec, numEquPerEnt_, targetSource, targetSourceIndex, numNeighbors_, targetSourceFactor, numTrgEntities);
...
void NearestNeighbourInterpolator::PrepareCalculation(){
  std::cout << "\t ---> NearestNeighbourInterpolator preparing for interpolation" << std::endl;
  
  std::cout << "\t\t 1/4 Obtaining source entities " << std::endl;
  uuids::uuid upRes = upResIds[0];
  inGrid_ = resultManager_->GetExtInfo(upRes)->ptGrid; 												// Points to grid (and information about grid)
  ResultManager::ConstInfoPtr inInfo = resultManager_->GetExtInfo(upRes);							// Copies informations
  scrMap_ = resultManager_->GetEqnMap(upRes); 														// Returns the equation map of a result/source
  numEquPerEnt_ = scrMap_->GetNumEqnPerEnt();														// Returns Equations per entity
  bool inElems = inInfo->definedOn == ExtendedResultInfo::ELEMENT;									// Checking if Infos are identical
  
  StdVector<CF::UInt> globSrcEntity;																// initialize Vector
  GetUsedMappedEntities(scrMap_, globSrcEntity, srcRegions_, inGrid_);								// Collects global entity numbers (nodeNums or elemNums), according to the specified regions of the grid for the source values and on the source grid.
  const CF::UInt numSrcEntities = CountUsedEntities(globSrcEntity);									// Count the number of entities, which are used for the interpolation based on the source entities
  if (numSrcEntities < numNeighbors_) {
    numNeighbors_ = numSrcEntities;
  }
  
  trgMap_ = resultManager_->GetEqnMap(filterResIds[0]);												// Returns the entity map used for target values
  /*
  // checking, if interpolation matrix need to be created
  for (UInt i = 0; i < interpolators_.GetSize(); i++) {
  ...
  */
  interpolators_.Push_back(this);																	// contains pointers to every interpolator which created a matrix
  matrixIndex_ = matrices_.GetSize();																// contains the sizes of the matrices created by the Interpolators from interpolators_
  matrices_.Resize(matrixIndex_ + 1);																// Resizes size of matrixIndex by +1
  Matrix& matrix = matrices_[matrixIndex_];															// ....................
  
  std::cout << "\t\t 2/4 Obtaining target entities " << std::endl;
  
  StdVector<CF::UInt> globTrgEntity;																// initializing globTrgEntity vector
  GetUsedMappedEntities(trgMap_, globTrgEntity, trgRegions_, trgGrid_);								// Collects global entity numbers (nodeNums or elemNums), according to the specified regions of the grid for target values and on the target grid
  const CF::UInt numTrgEntities = CountUsedEntities(globTrgEntity);									// Count the number of entities, which are used for the interpolation - target nodes
    
  std::cout << "\t\t\t Interpolator is dealing with " << numSrcEntities <<
               " source " << (inElems ? "elements" : "nodes") << " and "<< numTrgEntities << " target nodes" << std::endl;

  std::cout << "\t\t 3/4 Creating search tree " << std::endl;
  std::vector<Point_3> points;																		// initialize Vector
  std::vector<CF::UInt> indices;  																	// initialize Vector
  for(CF::UInt srcEnt = 0; srcEnt < numSrcEntities; srcEnt++) {
    CF::UInt globEntityNumber = globSrcEntity[srcEnt];												// Delivers entity number which is in use
    if (globEntityNumber != UnusedEntityNumber) {													// checking if selected entity is in use (therefore inside the ROI)
      CF::Vector<Double> pCoord;																	// initialize Vector
      if (inElems) {
        inGrid_->GetElemCentroid(pCoord,globEntityNumber,true);										// Extract Center of Element
      } else {        inGrid_->GetNodeCoordinate3D(pCoord, globEntityNumber);						// Get coordinates of node (dimension: 3D)
      }
      Point_3 point(pCoord[0],pCoord[1],pCoord[2]);													// Assign detected coordinates
      points.push_back(point);																		// Stacks points to the end of vector
      indices.push_back(srcEnt);																	// Stacks used source entity number
    }
  }
  Tree tree(boost::make_zip_iterator(boost::make_tuple( points.begin(),indices.begin() )),
            boost::make_zip_iterator(boost::make_tuple( points.end(),indices.end() ) )
  ...
  std::cout << "\t\t 4/4 Creating interpolation matrix " << std::endl;
  matrix.numTargets = numTrgEntities;
  StdVector<CF::UInt>& targetSourceIndex = matrix.targetSourceIndex;
  StdVector<CF::UInt>& targetSource = matrix.targetSource;
  StdVector<CF::Double>& targetSourceFactor = matrix.targetSourceFactor;
  
  // creating target -> source indices
  targetSourceIndex.Resize(numTrgEntities + 1);
  CF::UInt index = 0;
  for(CF::UInt trgEnt = 0; trgEnt < numTrgEntities; trgEnt++) {
    targetSourceIndex[trgEnt] = index;
    if (globTrgEntity[trgEnt] != UnusedEntityNumber) {
      index += numNeighbors_;
    }
  }
  targetSourceIndex[numTrgEntities] = index;
