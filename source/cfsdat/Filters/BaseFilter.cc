...
void BaseFilter::InitResultsUpstream(){
  ...
  std::set<uuids::uuid>::iterator aIter = activeResults.begin();
  for(; aIter != activeResults.end(); ++aIter){
	std::cout << "DDDDDDDDDDDDDDDDDDDDDDDEACTIVATE: " << resultManager_->GetResultName(*aIter) << std::endl;
    resultManager_->DeactivateResult(*aIter);

  }
  // activate upstream results
  for(UInt aRes=0;aRes<upResIds.GetSize();aRes++){
	  std::cout << "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAActivate: " << resultManager_->GetResultName(upResIds[aRes]) << std::endl;		//////////
    resultManager_->ActivateResult(upResIds[aRes]);
  }
  // Going further upstream
  for(UInt aSrc=0;aSrc< sources_.GetSize(); aSrc++){
...
void BaseFilter::ExtractFilterResults(){
  std::set<uuids::uuid> activeResults = resultManager_->GetActiveResults(); //get id's of active results
  std::set<uuids::uuid>::iterator aIter = activeResults.begin();
  for(; aIter != activeResults.end(); ++aIter){                        //loop over active results
//	std::cout << "AAAAAAAAAAAAAAAAAAAAAAAACTIVE Results: " << resultManager_->GetResultName(*aIter) << std::endl;
    uuids::uuid aId = *aIter;
    ResultManager::ConstInfoPtr aInfo = resultManager_->GetExtInfo(aId);
    //print_ConstExtInfoFields((*aInfo.get()));
...
bool BaseFilter::Run() {                     //gets obsolete results and calculates results, at the end updates result vector
  std::set<uuids::uuid> obsoleteResults = ExtractObsoleteResults();
  std::set<uuids::uuid>::iterator sIter = obsoleteResults.begin();

  //for (; sIter != obsoleteResults.end(); ++sIter) { // for testing   obsoleteResults is acouRhsLoad and fluidMechPressure
	//  std::cout << "run obsolete: " << resultManager_->GetResultName(*sIter) << std::endl;
	//  std::cout << resultManager_->GetResultName(*sIter) << std::endl;
  //}
