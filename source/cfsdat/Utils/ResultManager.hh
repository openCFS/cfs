...
//  Vector<Complex>& GetResultVector(uuids::uuid requestedId, CF::StdVector<UInt>& eqnNumbers);
  ...
  //implementiert von Clemens



  void ShowActiveResult(){  																									 //for testing
	  std::cout << "Show active Results" << std::endl;
	  std::set<uuids::uuid> activeResults = this->GetActiveResults();
	  std::set<uuids::uuid>::iterator aIter = activeResults.begin();
	  for(; aIter != activeResults.end(); ++aIter){                        //loop over active results
		  //std::cout << "testing" << std::endl;
	    uuids::uuid aId = *aIter;
	    std::cout << "Active Result: " << this->GetResultName(aId) << std::endl;
	  }
  }

  	  	  	  	  	  	  	  	  	  	  	  	  	  	  	  // for testing
  std::vector<string> GetListofResultsName(){
	  //std::cout << "Get list of active results at the beginning" << std::endl;
	  std::set<uuids::uuid> activeResults = this->GetActiveResults();
	  std::set<uuids::uuid>::iterator aIter = activeResults.begin();
	  std::vector<string> ActiveStartingResults;
	  //std::set<boost::uuids::uuid> ActiveStartingResults;
	  for(; aIter != activeResults.end(); ++aIter){
		  uuids::uuid aId = *aIter;
		  //std::cout << " \t \t Not calculated values: " << GetResultName(aId) << "  ID: " << aId << std::endl;
		  //ActiveStartingResults.insert(aId);
		  ActiveStartingResults.push_back(GetResultName(aId));
	  }
	  return ActiveStartingResults;
  }

  std::set<boost::uuids::uuid> GetListofResultsid(){
	  //std::cout << "Get list of active results at the beginning" << std::endl;
	  std::set<uuids::uuid> activeResults = this->GetActiveResults();
	  std::set<uuids::uuid>::iterator aIter = activeResults.begin();
	  std::set<boost::uuids::uuid> ActiveStartingResultsid;
	  for(; aIter != activeResults.end(); ++aIter){
		  uuids::uuid aId = *aIter;
		  //std::cout << " \t \t Not calculated values: " << GetResultName(aId) << "  ID: " << aId << std::endl;
		  ActiveStartingResultsid.insert(aId);
	  }
	  return ActiveStartingResultsid;
  }

  std::set<boost::uuids::uuid> TestMethod(){
	  std::set<boost::uuids::uuid> AllresultMapResults;
	  boost::unordered_map<uuids::uuid,std::pair<InfoPtr,ResPtr> >::iterator it;
	  for (it=resultMap_.begin();it!=resultMap_.end();++it){
		  //std::cout << it->first.data <<", " <<std::endl;
		  //uuids::uuid itd = *it;
		  //AllresultMapResults.insert(it);
	  }
	  return AllresultMapResults;
  }
  ...
  void ActivateResult(uuids::uuid toActive){
    ...
    std::cout << "TooooooooooooooooooooooActive: " << toActive << std::endl;
  ...
  void DeactivateResult(uuids::uuid toSleep){
    ...
    std::cout << "TooooooooooooooooooooooSleeeeeeeeeeeeep: " << GetResultName(toSleep) << std::endl;
