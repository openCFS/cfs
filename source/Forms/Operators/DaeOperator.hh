...
namespace CoupledField{
 ...
 //! This class implements the identity operator, which gets projected in 
  //! a prescribed direction w. The element matrix is computed as 
  //!
  //! b = ( N_1*w_x + N_1*w_y + N_1*w_z  N_2*w_x + N_2*w_y + N_2*w_z  ..  ) = scalarproduct(\vec{N}, \vec{w}
  //!
  //! and is of size (1 x Number of functions (DOF)).
  template<class FE, UInt D , UInt D_DOF = 1, class TYPE = Double>
  class DaeIdentityOperatorProjected : public BaseBOperator{
  ...
  public:
    ...
    //! Copy constructor
    DaeIdentityOperatorProjected(const DaeIdentityOperatorProjected & other)
       : BaseBOperator(other){
    }

    //! \copydoc BaseBOperator::Clone()
    virtual DaeIdentityOperatorProjected * Clone(){
      return new DaeIdentityOperatorProjected(*this);
    }

    virtual ~DaeIdentityOperatorProjected(){
      return;
    }
  ...
  // ===============================================
  //  DaeIdentityOperatorProjected FOR H-CURL ELEMENTS
  // ===============================================
  //! Partial specialized class for HCurl elements
  ...
  //! a prescribed direction w. The element matrix is computed as 
  //!
  //! b = ( N_1*w_x + N_1*w_y + N_1*w_z  N_2*w_x + N_2*w_y + N_2*w_z  ..  ) = scalarproduct(\vec{N}, \vec{n})
  //!
  //! and is of size (1 x Number of functions (DOF)).
  template<UInt D , UInt D_DOF, class TYPE>
  class DaeIdentityOperatorProjected<FeHCurl, D, D_DOF, TYPE> : public BaseBOperator{
  public:
  ...
  template<UInt D, UInt D_DOF, class TYPE>
  void DaeIdentityOperatorProjected<FeHCurl,D,D_DOF,TYPE>::
  CalcOpMat(Matrix<Double> & bMat,
            const LocPointMapped& lp, BaseFE* ptFe){
