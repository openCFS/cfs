...
namespace CoupledField {
  ...
  SurfCurrentLinForm::SurfCurrentLinForm( const std::string& value,
                                       const std::string& phase,
                                       const std::string& subType,
                                       bool isAxi )
    : LinearSurfForm() {

    name_ = "SurfCurrentLinForm";
    isaxi_ = isAxi;
  ...
  SurfCurrentLinForm::~SurfCurrentLinForm() {
  }

  void SurfCurrentLinForm::GetSurfCurrentFactor(EntityIterator& ent,
                                                Vector<Double>& locPoint,
                                                Double& factor) {
    // register global coordinates of element midpoint
    const SurfElem* elem = ent.GetSurfElem();

    RegisterSurfElemPoint( mHandle_, elem, locPoint );
  ...
  void SurfCurrentLinForm::CalcElemVector( Vector<Double>& elemVec, 
                                           EntityIterator& ent ) {
    // Extract pointer to reference element and get coordinates
    ExtractElemInfo( ent );
    ...
    for (UInt actIntPt=1; actIntPt <= nrIntPts; actIntPt++)
      {
        ...
        GetSurfCurrentFactor(ent, intPoints[actIntPt-1], val); 

        Double factor = intWeights[actIntPt-1] * jacDet * val;
  ...
  void SurfCurrentLinForm::CalcElemVector( Vector<Complex>& elemVec, 
                                           EntityIterator& ent ) {
    ...
    for (UInt actIntPt=1; actIntPt <= nrIntPts; actIntPt++)
      {
        ...
        Complex factor = intWeights[actIntPt-1] * jacDet * val;
