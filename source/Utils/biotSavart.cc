...
DEFINE_LOG(bisa, "biotSavart")
  ...
  Vector<Double>& BiotSavart::CalcFieldAllEqns( bool normalized ) {
    ...
    // Omit re-calculation, if normalized field was already calculated
    if( !fieldIsComputed_) {
      ...
      for( ; coilIt != coils_.End(); coilIt++ ) {
        ...
        //allocate memory
        actCoil.fieldNormalized_.Resize(numEqns);
        actCoil.fieldNormalized_.Init();
        ...
        for ( UInt inode = 1; inode <= numNodes; inode++) {
          ...
          try {
            ...
            ComputeFieldNormalized( magVec, pCoord, actCoil );
          ...
          if ( dim_ == 3 ) {
            for ( UInt dof=1; dof<=3; dof++ ) {
              ...
              if ( eqn > 0 )
                actCoil.fieldNormalized_[eqn-1] += magVec[dof-1];
            }
          }
          else {
            UInt eqn = eqnMap_->GetNodeEqn( globNodeNr, 1);
            if ( eqn > 0)  
              actCoil.fieldNormalized_[eqn-1] += magVec[2];
          }      
        } // loop over all nodes
      } // loop over all coils

      // Resize also "the one" vector potential array
      field_.Resize( numEqns );
  ...
  void BiotSavart::LineIntegralHField( Vector<Double>& partMagVec, 
    ...
    // Double norm1; // TODO: Unused variable norm1
    Double norm2;
    Double norm3;
    Double dotProd13;
    Double dotProd12;
