...
namespace CoupledField {
  ...
  /**
   * @brief Reads the participant setup from a preCICE configuration file.
   *
   * Extracts, per participant, the provided/received meshes and the read-/write-data
   * entries from precice-config.xml (via MinimalXmlParser). The adapter uses this to
   * know which exchange quantities to register with the preCICE participant.
   */
