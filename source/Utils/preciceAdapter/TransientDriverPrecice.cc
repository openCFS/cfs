...
namespace CoupledField {
      ...
      TransientDriver(sequenceStep, isPartOfSequence, state, domain, paramNode, infoNode)
  {
    ...
    LOG_DBG(trans_driverprecice) << "Using TransientDriverPrecice";
    if(!preciceAdapter_)
  ...
  void TransientDriverPrecice::SolveProblem()
  {
    ...
    LOG_DBG(trans_driverprecice) << "actTimeStep_: " << actTimeStep_ << ", actTime_: " << actTime_
                                 << ", firstdt_: " << firstdt_ << ", initialTime_: " << initialTime_
                                 << ", endStep_: " << endStep_;
    ...
    while(preciceAdapter_->IsCouplingOngoing()){
      ...
      LOG_DBG(trans_driverprecice) << "cfs_dt: " << cfs_dt << ", precice_dt: " << precice_dt << ", using dt: " << dt;
