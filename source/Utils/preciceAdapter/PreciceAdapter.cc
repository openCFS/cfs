...
namespace CoupledField
{
  ...
  PreciceAdapter::PreciceAdapter(shared_ptr<ParamNode> paramNode)
  ...
  void PreciceAdapter::readConfigurationParameters() {
    ...
    sequenceStep_ = paramNode_->Get("fileFormats/preciceCoupling")->Get("sequenceStep")->As<UInt>();
    ...
    LOG_DBG(preciceAdapter) << "PreciceAdapter: Configuration parameters:";
    LOG_DBG(preciceAdapter) << "  Participant Name: " << participantName_;
    LOG_DBG(preciceAdapter) << "  Participant Mesh Entries: " << meshNodes.GetSize();
    LOG_DBG(preciceAdapter) << "  Precice Config File: " << configFileName_;
    LOG_DBG(preciceAdapter) << "  Sequence Step: " << sequenceStep_;
  }

  void PreciceAdapter::readPreciceConfiguration() {
    LOG_DBG(preciceAdapter) << "PreciceAdapter: reading precice configuration from file: " << configFileName_;
  ...
  void PreciceAdapter::createPreciceParticipant() {
    LOG_DBG(preciceAdapter) << "PreciceAdapter: creating PreCICE participant...";
    ...
    LOG_DBG(preciceAdapter) << "PreciceAdapter: created PreCICE participant '" << participantName_
    ...
    for (const auto &entry : activeParticipantConfig_.writeData) {
      ...
      LOG_DBG(preciceAdapter) << "PreciceAdapter: Preparing to register write-data entry: " << config.precicename
                << " on mesh: " << config.meshName
                << " with dimension: " << config.quantitydim;

      LOG_DBG(preciceAdapter) << "CFSName is: " << config.cfsname;
    ...
    LOG_DBG(preciceAdapter) << "PreciceAdapter: Registered " << runtimeWriteResults_.size() << " write-data entries.";
    ...
    for (const auto &entry : activeParticipantConfig_.readData) {
      ...
      LOG_DBG(preciceAdapter) << "PreciceAdapter: Preparing to register read-data entry: " << config.precicename
                << " on mesh: " << config.meshName
                << " with dimension: " << config.quantitydim;
      ...
      if (mappedType == ResultType::ELEMENT) {
        ...
        LOG_DBG(preciceAdapter) << "PreciceAdapter: Registered " << runtimeReadResults_.size() << " read-data entries.";
      }
    }
    ...
    for (auto& meshEntry : meshDataByName_) {
      ...
      if (md.needsNodeData) {
        ...
        LOG_DBG(preciceAdapter) << "PreciceAdapter: Registered nodal mesh '" << meshName
        ...
        LOG_DBG(preciceAdapter) << "PreciceAdapter: Registered element mesh '" << meshName
  ...
  void PreciceAdapter::setupMeshAndCoordinates() {
    LOG_DBG(preciceAdapter) << "PreciceAdapter: setting up mesh and coordinates...";
    ...
    for (const auto& meshName : usedMeshNames) {
      ...
      LOG_DBG(preciceAdapter) << "PreciceAdapter: prepared mesh '" << meshName
  ...
  void PreciceAdapter::Initialize(Domain *domain, SinglePDE* pde) {
    ...
    if(!isInit_){
      ...
      if(!tp || !(tp->GetActSequenceStep() == sequenceStep_)){
        LOG_DBG(preciceAdapter) << "PreciceAdapter: Not the correct sequence step, skipping precice initialization";
        return;
      }
      ...
      LOG_DBG(preciceAdapter) << "Not yet initialized";

      createPreciceParticipant();

      LOG_DBG(preciceAdapter) << "PreciceAdapter: Precice participant created.";
      ...
      if(participant_->requiresInitialData()){
        LOG_DBG(preciceAdapter) << "PreciceAdapter: Participant requires initial data - initializing 'Temperature' to 293.15K";
  ...
  void PreciceAdapter::RegisterSinglePDE(SinglePDE* pde){
    ...
    if (std::find(pdes_.begin(), pdes_.end(), pde) == pdes_.end()) {
      pdes_.push_back(pde);
      LOG_DBG(preciceAdapter) << "PreciceAdapter: Registered SinglePDE (" << pdes_.size() << " total).";
    }
  }
  ...
  void PreciceAdapter::RegisterTimeStepWriteData(){
    ...
    // Build a vector of contexts using shared_ptr.
    std::vector<std::pair<shared_ptr<BaseResult>, shared_ptr<ResultHandler::ResultContext>>> contexts;
    for (auto &entry : *resultContextsPtr) {
    ...
    for (auto &result : runtimeWriteResults_) {
      ...
      if (result->getResultType() == ResultType::NODE) {
        NodeResult* nr = dynamic_cast<NodeResult*>(result.get());
        LOG_DBG(preciceAdapter) << "writing NODE-based result";
        if (nr) {
        ...
        ElementResult* er = dynamic_cast<ElementResult*>(result.get());
        LOG_DBG(preciceAdapter) << "writing ELEMENT-based result";
        if (er) {
  ...
  void PreciceAdapter::MarkReadResultsUpdated()
  {
    ...
    for (auto &entry : *resultContextsPtr) {
      ...
      for (const auto &result : runtimeReadResults_) {
        if (result->getConfig().cfsname == cfsResultName) {
          LOG_DBG(preciceAdapter) << "PreciceAdapter::MarkReadResultsUpdated: marking '"
  ...
  void PreciceAdapter::RegisterExternalResults()
  {
    ...
    LOG_DBG(preciceAdapter) << "PreciceAdapter: Registering external results with OpenCFS result handler...";

    ResultHandler* resHandler = domain_->GetResultHandler();
    if(!resHandler)
        EXCEPTION("PreciceAdapter::RegisterExternalResults: No result handler available.");

    LOG_DBG(preciceAdapter) << "runtimeReadResults.size() = " << runtimeReadResults_.size();
