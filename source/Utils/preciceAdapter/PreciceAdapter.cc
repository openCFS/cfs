...
void PreciceAdapter::createPreciceParticipant()
{
  ...
  for (const auto &entry : activeParticipantConfig_.writeData) {
    ...
    config.quantitydim = (needElemMesh_) ? participant_->getDataDimensions(participantMeshName_ + "_elem", config.precicename) : participant_->getDataDimensions(participantMeshName_, config.precicename);
...
{
  for (const auto &result : runtimeReadResults_) {
    std::cout << "[PreciceAdapter]: solutiontype: " << result->getConfig().solutiontype << "\n";
    std::cout << "[PreciceAdapter]: solType: " << static_cast<int>(solType) << "\n";
    // std::cout << "[PreciceAdapter]: DO THEY MATCH?" << "\n";
    if (result->getConfig().solutiontype == solType) {
      if (result->getResultType() == ResultType::NODE) {
      ...
      std::cout << "[PreciceAdapter]: solutiontype != solType" << "\n";
  }
  EXCEPTION("PreciceAdapter: no result with solution type " << solType << " found");
}
