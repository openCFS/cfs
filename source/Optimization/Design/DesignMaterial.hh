...
public:
  ...
  typedef enum {
    ...
    TRANSVERSAL_ISOTROPIC_BOXED,
    DENSITY_TIMES_TRANSVERSAL_ISOTROPIC,
    DENSITY_TIMES_TRANSVERSAL_ISOTROPIC_BOXED,
    DENSITY_TIMES_ROT_TRANSVERSAL_ISOTROPIC_BOXED,
    DENSITY_TIMES_2D_TENSOR,
    DENSITY_TIMES_2D_TENSOR_CONSTANT_TRACE,
    DENSITY_TIMES_ROTATED_2D_TENSOR,
    LAMINATES,
    HOM_RECT,
    HOM_RECT_C1
  } Type;
  ...
  typedef enum {
    TRANSISO_XY, TRANSISO_YZ, TRANSISO_XZ
  } TransIsoType;
  ...
  DesignMaterial(PtrParamNode pn, OptimizationMaterial::System material,
  ...
  bool HasParameter(const DesignElement::Type p) const {
    return params_.find(p) != params_.end();
  }
  ...
  bool GetMaterialDamping(double& alpha, double& beta,
      DesignElement::Type direction = DesignElement::NO_DERIVATIVE);
...
private:
  ...
  inline void GetIsoMaterialTensor(Matrix<double>& t, SubTensorType subTensor,
      DesignElement::Type direction);

  /** Calculate the Lame Tensor */
  inline void GetLameMaterialTensor(Matrix<double>& t, SubTensorType subTensor,
      DesignElement::Type direction);

  /** Calculate the Trans-Iso Tensor */
  inline void GetTransIsoMaterialTensor(Matrix<double>& t,
      SubTensorType subTensor, DesignElement::Type direction);

  /* general anisotropic FMO tensor */
  inline void GetAnisotropicTensor(Matrix<double>& t,
      DesignElement::Type direction, Notation notation);

  /** Calculate the Tensor for Density times Tensor */
  inline void GetDensityTimes2dTensorTensor(Matrix<double>& t,
      SubTensorType subTensor, DesignElement::Type direction);

  /** Calculate the tensor for Laminates */
  inline void GetLaminatesTensor(Matrix<double>& t, SubTensorType subTensor,
      DesignElement::Type direction, Notation notation);
  ...
  void ApplyHomRectC1Tensor(Matrix<double>& E, Vector<double>& p,
  ...
  inline void GetHomRectTensor(Matrix<double>& t, SubTensorType subTensor,
      DesignElement::Type direction, Notation notation);
  ...
  inline void Set2dVoigtTensor(Matrix<double>& t, double t11, double t22,
      double t33, double t23, double t13, double t12);

  /** put the entries of the transversal_isotropic tensor at the right places */
  inline void SetTransIsoTensor(Matrix<double>& t, SubTensorType subTensor,
      double iD, double inD, double iG, double oD, double onD, double oG);

  /** put the entries of the isotropic tensor at the right places */
  inline void SetIsoTensor(Matrix<double>& t, SubTensorType subTensor, double D,
      double nD, double G);
  ...
  void FillHomRectSamples(PtrParamNode homRect, unsigned int idx,
  ...
  double EvaluateC1Interpolation(Matrix<double>&, Vector<double>&,
      const Matrix<double>&, double &, double &, int &, int &, int &,
      int &) const;
  ...
  double EvaluateC1Interpolation_Deriv(Matrix<double>&, Vector<double>& p,
      const Matrix<double>&, double &, double &, int &, int &, int &, int &,
      DesignElement::Type direction) const;

  double EvaluateC1Interpolation_3D(Matrix<double>&, Vector<double>&,
      const Matrix<double>&, double &, double &, double &, int &, int &, int &,
      int &, int &, int &) const;
  /** evaluates the derivative of the C1 interpolation polynomial at point p[0],p[1] in direction 0 or 1 and returns function value as double */
  double EvaluateC1Interpolation_Deriv_3D(Matrix<double>&, Vector<double>& p,
      const Matrix<double>&, double &, double &, double &, int &, int &, int &,
      int &, int &, int &, DesignElement::Type direction) const;
