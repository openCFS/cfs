...
namespace CCM {
  ...
  void MeshFile::ReadFaces(std::vector<FaceData>* faceDatas) {
    ...
    for (uint iTopology = 0; iTopology < topologies_.size(); iTopology++) {
      ...
      for (uint iFaces = 0; iFaces < facesIDs.size(); iFaces++) {
        ...
        CCMIOEntitySize(&err_, facesID, &faceData.size, NULL);
        std::cout << "    Faces: " << faceData.label << " (id: " << faceData.id << ", size: " << faceData.size << ")" << std::endl;
        
        CCMIOID mapID;
        CCMIOReadFaces(&err_, facesID, kCCMIOInternalFaces, &mapID, &faceData.faceVerticesSize, NULL, kCCMIOStart, kCCMIOEnd);
        ...
        CCMIOReadFaces(&err_, facesID, kCCMIOInternalFaces, NULL, NULL, faceData.faceVertices, kCCMIOStart, kCCMIOEnd);
        faceData.faceCells = new int[faceData.size * 2];
        CCMIOReadFaceCells(&err_, facesID, kCCMIOInternalFaces, faceData.faceCells, kCCMIOStart, kCCMIOEnd);
      ...
      for (uint iFaces = 0; iFaces < facesIDs.size(); iFaces++) {
        ...
        CCMIOID mapID;
        CCMIOReadFaces(&err_, facesID, kCCMIOBoundaryFaces, &mapID, &faceData.faceVerticesSize, NULL, kCCMIOStart, kCCMIOEnd);
        faceData.mapID = mapID.id;
        OpenLabel(mapID, faceData.mapLabel);
        std::cout << "      Map: " << faceData.mapLabel << " (id: " << faceData.mapID << ")" << std::endl;
        std::cout << "      Numer of Edges: " << (faceData.faceVerticesSize - faceData.size) << std::endl;
        faceData.faceVertices = new int[faceData.faceVerticesSize];
        CCMIOReadFaces(&err_, facesID, kCCMIOBoundaryFaces, NULL, NULL, faceData.faceVertices, kCCMIOStart, kCCMIOEnd);
        faceData.faceCells = new int[faceData.size];
        CCMIOReadFaceCells(&err_, facesID, kCCMIOBoundaryFaces, faceData.faceCells, kCCMIOStart, kCCMIOEnd);
        faceDatas->push_back(faceData);
        std::cout << "      Completed" << faceData.mapLabel << std::endl;
      }
    }
  ...
  void MeshFile::ReadVertices(std::vector<VertexData>* vertexDatas) {
    ...
    for (uint iVertices=0; iVertices < verticesIDs.size(); iVertices++) {
      ...
      CCMIOEntitySize(&err_, verticesID, &vertexData.size, NULL);
      ...
      CCMIOReadVerticesd(&err_, verticesID, &vertexData.dim, &vertexData.scale, &mapID, 
              vertexData.coordinates, kCCMIOStart, kCCMIOEnd);
  ...
  void MeshFile::ReadCells(std::vector<CellData>* cellDatas) {
    ...
    for (uint iTopology = 0; iTopology < topologies_.size(); iTopology++) {
      ...
      for (uint iCells = 0; iCells < cellsIDs.size(); iCells++) {
        ...
        CCMIOEntitySize(&err_, cellsID, &cellData.size, NULL);
        std::cout << "    Cells: " << cellData.label << " (id: " << cellData.id << ", size: " << cellData.size << ")" << std::endl;
        
        CCMIOID mapID;
        CCMIOReadCells(&err_, cellsID, &mapID, NULL, kCCMIOStart, kCCMIOEnd);
