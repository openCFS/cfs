...
namespace CoupledField
{
                                         ...
                                         const UInt timeStepIdx)
    {
        ...
        for(int n=0; n<nvx; n++)
        {
          if(settings.GetInt("floatDataset"))
          {
            flowdata[n*7+1] = floatvec[n*3+0];
            flowdata[n*7+2] = floatvec[n*3+1];
            flowdata[n*7+3] = floatvec[n*3+2];
          }
          else
          {
    ...
    void FileReader_CFX::GetInfosFromCommand()
    {
      std::string cmd, attrib;
        ...
        sstr.str("");
        sstr << settings.GetString("name") << "_data/UserData_CFX_COMMANDS.dat";
        fileName = sstr.str();
        outFile.open(fileName.c_str(),
                     std::ofstream::out | std::ofstream::trunc);

        ParseCommand(charvec, pos, cmd, attrib, "", outFile);
        ParseCommand(charvec, pos, cmd, attrib, "", outFile);
        ParseCommand(charvec, pos, cmd, attrib, "", outFile);
        ParseCommand(charvec, pos, cmd, attrib, "", outFile);
        ParseCommand(charvec, pos, cmd, attrib, "", outFile);
