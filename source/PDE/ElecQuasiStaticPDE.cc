...
//  //set tensor type
  ...
  void ElecQuasistaticPDE::InitTimeStepping()
  {
    if (GetDomain()->GetAdaptiveData())
        EXCEPTION("Adaptive timestepping is not supported for ElecQuasistaticPDE: variable-step BDF2 requires C3-smooth solution history, which is not registered for this PDE. Use fixed deltaT.");
