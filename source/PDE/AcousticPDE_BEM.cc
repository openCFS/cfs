...
#include "Materials/AcousticMaterial.hh"                      // can we keep this?

namespace CoupledField
{
  DEFINE_LOG(acousticpde_bem, "pde.acoustic_bem")
  ...
  void AcousticPDE_BEM::ReadDampingInformation() {
  ...
  void AcousticPDE_BEM::DefineIntegrators(){
    ...
    LOG_DBG(acousticpde_bem) << "DefineIntegrators BEGIN" <<  "\n" ;
    ...
    for (UInt iRegion = 0; iRegion < regions_.GetSize(); iRegion++) {
      ...
      LOG_DBG(acousticpde_bem) << "DefineIntegrators : dens   = " << dens->ToString() << "\n";
      LOG_DBG(acousticpde_bem) << "DefineIntegrators : blk    = " << blk->ToString() << "\n";
      ...
      LOG_DBG(acousticpde_bem) << "DefineIntegrators Fluid: coeffK = " << coeffK->ToString() << "\n";
      LOG_DBG(acousticpde_bem) << "DefineIntegrators Fluid: coeffM = " << coeffM->ToString() << "\n";
      ...
      if( dampingList_[actRegion] == PML ) 
      {
        ...
        if(analysistype_ == HARMONIC || analysistype_ == BasePDE::INVERSESOURCE)
        {
          ...
          {
            EXCEPTION("Formulation '" << pmlFormul << "' for AcousticPDE_BEM " 
          ...
          {
            EXCEPTION("Unknown PML-formulation '" << pmlFormul << "' for AcousticPDE_BEM. " 
  ...
  template<UInt DIM>
  void AcousticPDE_BEM::DefineTransientPMLInts(shared_ptr<ElemList> eList, std::string id,
  ...
  void AcousticPDE_BEM::DefineNcIntegrators() {
//	if ( complexFluidFormulation_ )
//		EXCEPTION("Complex fluid and NC-interfaces currently not allowed");
  ...
  void AcousticPDE_BEM::DefineSurfaceIntegrators( ){
    //========================================================================================
    // ABC boundaries
    //========================================================================================
    PtrParamNode bcNode = myParam_->Get( "bcsAndLoads", ParamNode::PASS );
    LOG_DBG(acousticpde_bem) << "Define Surface Integrator BEGIN"<< "\n";

    if( bcNode ) {
      ParamNodeList abcNodes = bcNode->GetList( "absorbingBCs" );
      LOG_DBG(acousticpde_bem) << "ABCs count :  " << abcNodes.GetSize() <<  "\n" ;
      ...
      for( UInt i = 0; i < abcNodes.GetSize(); i++ ) {
        ...
        LOG_DBG(acousticpde_bem) << "ABCs volRegName :  " << volRegName <<  "\n" ;
        ...
        LOG_DBG(acousticpde_bem) << "ABC: dens = " << dens->ToString() << "\n";
        LOG_DBG(acousticpde_bem) << "ABC: blk  = " << blk->ToString() << "\n";
        ...
        if (tempId != "") {
          ...
          c0 = CoefFunction::Generate( mp_,  Global::REAL,
                           ...
                           CoefXpr::OP_SQRT) );
          LOG_DBG(acousticpde_bem) << "Define Surface Integrators standard c0 =" << c0->ToString() << "\n";
        }
        else {
        	if (complexFluidFormulation_ ) {
        ...
        LOG_DBG(acousticpde_bem) << "Def Surface Integrator:  c0 =" << c0->ToString() << "\n";
	  ...
	  LOG_DBG(acousticpde_bem) << "Def Surface Integrator: factor =" << factor->ToString() << "\n";
        ...
        LOG_DBG(acousticpde_bem) << "Define Surface Integrator: coeffDamp =" << coeffDamp->ToString() << "\n";
      ...
      for( UInt i = 0; i < blNodes.GetSize(); ++i ) {
        ...
        LOG_DBG(acousticpde_bem) << "Define Surface Integrator: regionName =" << regionName << "\n";
        LOG_DBG(acousticpde_bem) << "Define Surface Integrator: volRegName =" << volRegName << "\n";
        LOG_DBG(acousticpde_bem) << "Define Surface Integrator: rho0       =" << rho0->ToString() << "\n";
        LOG_DBG(acousticpde_bem) << "Define Surface Integrator: K          =" << K->ToString() << "\n";
        LOG_DBG(acousticpde_bem) << "Define Surface Integrator: k          =" << k->ToString() << "\n";
        ...
        LOG_DBG(acousticpde_bem) << "Define Surface Integrator boundary layer: coefM =" << coefM->ToString() << "\n";
        ...
        LOG_DBG(acousticpde_bem) << "Define Surface Integrator boundary layer: coefK =" << coefK->ToString() << "\n";
      } // boundary Layers
    } // end if ( bcNode )
    LOG_DBG(acousticpde_bem) << "Define Surface Integrator END"<< "\n";
  } // DefineSurfaceIntegrators

  void AcousticPDE_BEM::DefineRhsLoadIntegrators() {
    LOG_TRACE(acousticpde_bem) << "Defining rhs load integrators for acoustic PDE";
    ...
    // ===========================
    LOG_DBG(acousticpde_bem) << "Reading normal velocity"; 
    ...
    // ===========================
    LOG_DBG(acousticpde_bem) << "Reading total velocity";
    ...
    // ===========================
    LOG_DBG(acousticpde_bem) << "Reading normal acceleration";
    ...
    for( UInt i = 0; i < ent.GetSize(); ++i ) {
      ...
      this->rhsFeFunctions_[formulation_]->AddLoadCoefFunction(coef[i], ent[i]);
       ...
       // ================
       LOG_DBG(acousticpde_bem) << "Reading rhsDensityVector";
       ...
       //  Diss Freidhager 2022
       // ================
       LOG_DBG(acousticpde_bem) << "Reading BCVector";
...
//    		         currently not available");
  ...
  void AcousticPDE_BEM::DefineSolveStep(){
  ...
  void AcousticPDE_BEM::FinalizePostProcResults(){
    //first call base class method
    SinglePDE::FinalizePostProcResults();
  }

  void AcousticPDE_BEM::DefinePostProcResults(){
...
*/
   ...
   void AcousticPDE_BEM::CreateMeanFlowFunction(StdVector<std::string> dofNames){
  ...
  void AcousticPDE_BEM::InitTimeStepping(){
  ...
  void AcousticPDE_BEM::ComputeSOS(PtrCoefFct& c0, PtrCoefFct dens, PtrCoefFct blk,
...
//TODO 1 erase inconsistent coding style 1/c0^2
  ...
  void AcousticPDE_BEM::ComputeSOS_SQR(PtrCoefFct& cSQR, PtrCoefFct dens, PtrCoefFct blk,
		                           PtrCoefFct regionTemp, std::string tempId) {
...
template void AcousticPDE_BEM::DefineTransientPMLInts<2>(shared_ptr<ElemList>, std::string,
    RegionIdType actRegion, std::string tempId);
template void AcousticPDE_BEM::DefineTransientPMLInts<3>(shared_ptr<ElemList>, std::string,
    RegionIdType actRegion, std::string tempId);