...
//    bool is3d = domain_->GetParamRoot()->Get("domain")->Get("geometryType")->As<std::string>() == "3d";
    if ( !is3d_ ){
      EXCEPTION("MagEdgeAStarPDE is just implemented for 3D setups!");
    }
  ...
  void MagEdgeAStarPDE::DefineStandardIntegrators(){
    ...
    if (((analysistype_ == TRANSIENT)) && nonLin_ && (modelName_ != "nonlinearCurve"))
    {
    ...
    for(UInt iRegion = 0; iRegion < regions_.GetSize() ; iRegion ++){
      ...
      if (  nonLinTypes.Find(PERMEABILITY) != -1 ) {
        if (modelName_ == "invEBHysteresisModel") { // use inverse energy-based Hysteresis model
          ...
          stiffInt->SetName("K_tan = (dH/dB curl A, curl A'),nonlinear case, nonlinear subregion)");
        ...
        // define needed variables
        BaseBDBInt* stiffness_matrix = NULL;
        PtrCoefFct nu = NULL;

        // obtain reluctivity
        nu = actMat->GetScalCoefFnc(MAG_RELUCTIVITY_SCALAR, Global::REAL);

        //compute permeability
        PtrCoefFct constOne = CoefFunction::Generate( mp_, Global::REAL, "1.0");
        PtrCoefFct permeability = NULL;
        permeability = CoefFunction::Generate( mp_,  Global::REAL, CoefXprBinOp(mp_, constOne, nu, CoefXpr::OP_DIV ) );
        ...
        // ===============================
        if( dim_ == 3) { // 3D case
          stiffness_matrix = new BBInt<>(new  CurlOperator<FeHCurl,3, Double>(), nu,1.0, updatedGeo_) ;
        } else {
          EXCEPTION("For Edge elements, only 3D is possible!")
        }
        reluc_->AddRegion(actRegion, nu);
        stiffness_matrix->SetName("linear stiffness matrix: (nu curlA, curlA')");
        BiLinFormContext * stiffContext = new BiLinFormContext(stiffness_matrix, STIFFNESS );
        stiffContext->SetEntities( actSDList, actSDList );
        stiffContext->SetFeFunctions( feFunc, feFunc );
        assemble_->AddBiLinearForm( stiffContext );
        // Important: Add bdb-integrator to global list, as we need them later
        // for calculation of postprocessing results
        bdbInts_.insert( std::pair<RegionIdType, BaseBDBInt*>(actRegion,stiffness_matrix) );
      }
        // =================================================
        // Standard linear Mass Matrix, conductivity is linear
        // =================================================
        ...
        if ( analysistype_ == STATIC) {
          ...
          massInt->SetName("M = (gamma A,A'); STIFFNESS contribution (regularization)");
          ...
          massInt->SetName("M = (gamma A,A'); DAMPING contribution (mass matrix)");
        }
  ...
  void MagEdgeAStarPDE::DefineRhsLoadIntegrators(){
    LOG_TRACE(magEdgeAStarPde) << "Defining rhs load integrators for magEdgeAStarPde";
    ...
    for (UInt iRegion = 0; iRegion < regions_.GetSize(); iRegion++) {
        ...
        if (  nonLinTypes.Find(PERMEABILITY) != -1 ) {
          ...
          linearform_h_nl_curln->SetName("r_mag = (H(B),curl N'); nonlinear problem; nonlinear subregion RHS");
          ...
          if (modelName_ == "invEBHysteresisModel") { // NONLINEAR CASE BUT LINEAR REGION: \int H curlA'
            ...
            linearform_h_lin_curln->SetName("r_mag = (H,curl N'); nonlinear problem; linear subregion RHS");
  ...
  void MagEdgeAStarPDE::DefinePostProcResults() {
    ...
    shared_ptr<CoefFunctionMulti> ccdCoef(new CoefFunctionMulti(CoefFunction::VECTOR,dim_,1,isComplex_));
    ...
    shared_ptr<CoefFunctionMulti> tcdCoef(new CoefFunctionMulti(CoefFunction::VECTOR, dim_,1,isComplex_));
...
for( ; regIt != regions_.End(); ++regIt ) {
  ...
  LinearForm* MagEdgeAStarPDE::GetRHSMagnetizationInt( Double factor, PtrCoefFct rhsMag, bool fullEvaluation ){
    LinearForm * lin = NULL;
    return lin;
  }

  BaseBDBInt* MagEdgeAStarPDE::GeHystStiffInt( Double factor, PtrCoefFct tensorReluctivity ){
    BaseBDBInt* stiffInt = NULL;
    return stiffInt;
  }
  ...
  MagEdgeAStarPDE::CreateFeSpaces(const std::string& formulation,
                             PtrParamNode infoNode ) {
