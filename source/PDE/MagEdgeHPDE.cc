...
DEFINE_LOG(magEdgeHPde, "magEdgeHPde")
  ...
  void MagEdgeHPDE::DefineStandardIntegrators(){
    ...
    for(UInt iRegion = 0; iRegion < regions_.GetSize() ; iRegion ++){
      ...
      feFunc->AddEntityList( actSDList );
        ...
        // ===============================
        PtrCoefFct curCoef = actMat->GetScalCoefFnc(MAG_RELUCTIVITY_SCALAR, Global::REAL);
        ...
        curlcurl = new BBInt<>(new  CurlOperator<FeHCurl,3, Double>(), permeability,1.0, updatedGeo_) ;
      ...
      }// lin mass/stiffness matrix part
    } // end for regions
  }
  ...
  void MagEdgePDE::DefineRhsLoadIntegrators() {
    LOG_TRACE(magEdgeHPde) << "Defining rhs load integrators for magEdgeHPDE";

    // Get FESpace and FeFunction of mechanical displacement
    shared_ptr<BaseFeFunction> myFct = feFunctions_[MAG_FIELD_INTENSITY];
