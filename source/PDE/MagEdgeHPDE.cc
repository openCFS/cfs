...
DEFINE_LOG(magEdgeHPde, "magEdgeHPde")
      ...
      if (is_pseudo_time_stepping == 1){
        ...
        if (nonLinTypes.Find(PERMEABILITY) != -1){ // NONLINEAR/HYSTERESIS CASE
          if (modelName_ == "JilesAthertonModel"){ // just because, nobody wonders that it does not work with this model
            ...
            /* max_svd = nlScalCoef_;
            PtrCoefFct factor = CoefFunction::Generate(mp_, Global::REAL,lexical_cast<std::string>(std::pow(10,penaltyParameter/2)));
            rho_art_nl = CoefFunction::Generate( mp_,  Global::REAL, CoefXprBinOp(mp_, max_svd, factor, CoefXpr::OP_MULT ) ); */
            curlcurl = new BBInt<>(new  CurlOperator<FeHCurl,3, Double>(), nlScalCoef_,1.0, updatedGeo_);
  ...
  void MagEdgeHPDE::DefineRhsLoadIntegrators(){
    ...
    LinearForm * lin1_pts = NULL; // (rho_art curlh,curlN), only in the nonlinear case necessary (PSEUDO TIME STEPPING (pts))
    ...
    for (UInt iRegion = 0; iRegion < regions_.GetSize(); iRegion++) {
      ...
      if (is_pseudo_time_stepping == 1){
        if(nonLin_ && (modelName_ == "EBHysteresisModel")) {
          ...
          // ===============================================================================================
          // lin1_pts: (rho_art curlh,curlN) [START]
          ...
          if (nonLinTypes.Find(PERMEABILITY) != -1){ // NONLINEAR CASE, NONLINEAR SUBREGION
            ...
            lin1_pts = new BUIntegrator<Double>(new CurlOperator<FeHCurl, 3,Double>(),-1.0, rho_times_curlh, volRegions, coefUpdateGeo);
            lin1_pts->SetName("(rho_art_nl curlh,curlN): residual");
            ...
            PtrCoefFct rho_times_curlh = CoefFunction::Generate(mp_, Global::REAL, temp);
            lin1_pts = new BUIntegrator<Double>(new CurlOperator<FeHCurl, 3,Double>(),-1.0, rho_times_curlh, volRegions, coefUpdateGeo);
            lin1_pts->SetName("(rho_art curlh,curlN): residual");
          }
          ...
          // ===============================================================================================
          // lin1_pts: (rho_art curlh,curlN) [END]
          // ===============================================================================================
