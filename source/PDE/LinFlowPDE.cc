...
namespace CoupledField
{
      ...
      : SinglePDE(grid, paramNode, infoNode, simState, domain)
  {
    ...
    for (UInt i = 0; i < regionNodes.GetSize(); i++)
    {
      ...
      if (std::find(presSurfaces_.Begin(), presSurfaces_.End(), actRegionId) != presSurfaces_.End())
      {
    ...
    if (enableC2_)
      WARN("ENABLING SECOND CONVECTIVE TERM (C2)");

    enableC3_ = myParam_->Get("enableC3")->As<bool>();
    if (enableC3_)
      WARN("ENABLING Third CONVECTIVE TERM (C3 = V0.Grad(V0)");
  }
  ...
  void LinFlowPDE::DefineIntegrators()
  {
    ...
    for (it = materials_.begin(); it != materials_.end(); it++)
    {
      ...
      if (isCompressible_)
      { // we need to subtract 2*mu/3 Div(v)I and add the bulk part lambda*Div(v)I
                                         ...
                                         shared_ptr<CoefFunctionMulti> additionalCoef)
  {
    ...
    // Bool to manually switch between implementations (sym. and not sym. - will be used for testing effects of symmetrization)
    bool use_sym_form = false;
    ...
    if (isMaterialComplex_)
    {
      penalty_v1_u1 = new SurfaceNitscheABInt<Complex, Complex>(new SurfaceIdentityOperator<FeH1, DIM, D_DOF>(),
                                                                new SurfaceIdentityOperator<FeH1, DIM, D_DOF>(),
                                                                shearViscosity, beta, curcpl, updatedGeo_, true, true);
    }
    else
    {
      penalty_v1_u1 = new SurfaceNitscheABInt<Double, Double>(new SurfaceIdentityOperator<FeH1, DIM, D_DOF>(),
                                                              new SurfaceIdentityOperator<FeH1, DIM, D_DOF>(),
                                                              shearViscosity, beta, curcpl, updatedGeo_, true, true);
    }
    ...
    if (isMaterialComplex_)
    {
      flux1_v1_p1 = new SurfaceNitscheABInt<Complex, Complex>(new SurfaceIdentityOperator<FeH1, DIM, D_DOF>(),
                                                              new SurfaceNormalOperator<FeH1, DIM, P_DOF>(),
                                                              constOne, 1.0, curcpl, updatedGeo_, true);
    }
    else
    {
      flux1_v1_p1 = new SurfaceNitscheABInt<Double, Double>(new SurfaceIdentityOperator<FeH1, DIM, D_DOF>(),
                                                            new SurfaceNormalOperator<FeH1, DIM, P_DOF>(),
                                                            constOne, 1.0, curcpl, updatedGeo_, true);
    }
    // v1 p1 round two
    if (use_sym_form)
    {
      EXCEPTION("Integrator v1 p1 for symmetrization not implemented yet!")
    }
    ...
    if (use_sym_form)
    {
      ...
      if (isMaterialComplex_)
      {
        flux2_dv1_u1 = new SurfaceNitscheABInt<Complex, Complex>(new SurfaceNormalStressOperator<FeH1, DIM, D_DOF>(subType, false),
                                                                 new SurfaceIdentityOperator<FeH1, DIM, D_DOF>(),
                                                                 constOne, -1.0, curcpl, updatedGeo_, true);
      ...
      {
        flux2_dv1_u1 = new SurfaceNitscheABInt<Double, Double>(new SurfaceNormalStressOperator<FeH1, DIM, D_DOF>(subType, false),
                                                               new SurfaceIdentityOperator<FeH1, DIM, D_DOF>(),
                                                               constOne, -1.0, curcpl, updatedGeo_, true);
        flux2_dv1_u1->SetBCoefFunctionOpA(coefBB);
      }
    ...
    if (isMaterialComplex_)
    {
      flux2_v1_du1 = new SurfaceNitscheABInt<Complex, Complex>(new SurfaceIdentityOperator<FeH1, DIM, D_DOF>(),
    ...
    {
      flux2_v1_du1 = new SurfaceNitscheABInt<Double, Double>(new SurfaceIdentityOperator<FeH1, DIM, D_DOF>(),
                                                             new SurfaceNormalStressOperator<FeH1, DIM, D_DOF>(subType, false),
                                                             constOne, -1.0, curcpl, updatedGeo_, true);
      flux2_v1_du1->SetBCoefFunctionOpB(coefBB);
    }
    ...
    if (use_sym_form)
    {
      ...
      if (isMaterialComplex_)
      {
        flux3_dv1_u1 = new SurfaceNitscheABInt<Complex, Complex>(new SurfaceNormalDivOperator<FeH1, DIM>(),
                                                                 new SurfaceIdentityOperator<FeH1, DIM, D_DOF>(),
                                                                 coefDivDiv, -1.0, curcpl, updatedGeo_, true);
      }
      else
      {
        flux3_dv1_u1 = new SurfaceNitscheABInt<Double, Double>(new SurfaceNormalDivOperator<FeH1, DIM>(),
                                                               new SurfaceIdentityOperator<FeH1, DIM, D_DOF>(),
                                                               coefDivDiv, -1.0, curcpl, updatedGeo_, true);
      }
    }
    ...
    if (isMaterialComplex_)
    {
      flux3_v1_du1 = new SurfaceNitscheABInt<Complex, Complex>(new SurfaceIdentityOperator<FeH1, DIM, D_DOF>(),
                                                               new SurfaceNormalDivOperator<FeH1, DIM>(),
                                                               coefDivDiv, -1.0, curcpl, updatedGeo_, true);
    }
    else
    {
      flux3_v1_du1 = new SurfaceNitscheABInt<Double, Double>(new SurfaceIdentityOperator<FeH1, DIM, D_DOF>(),
                                                             new SurfaceNormalDivOperator<FeH1, DIM>(),
                                                             coefDivDiv, -1.0, curcpl, updatedGeo_, true);
    }
    ...
    if (isMaterialComplex_)
    {
      penalty_v1_u2 = new SurfaceNitscheABInt<Complex, Complex>(new SurfaceIdentityOperator<FeH1, DIM, D_DOF>(),
                                                                new SurfaceIdentityOperator<FeH1, DIM, D_DOF>(),
                                                                shearViscosity, beta * -1.0, curcpl, updatedGeo_, true, true);
    }
    else
    {
      penalty_v1_u2 = new SurfaceNitscheABInt<Double, Double>(new SurfaceIdentityOperator<FeH1, DIM, D_DOF>(),
                                                              new SurfaceIdentityOperator<FeH1, DIM, D_DOF>(),
                                                              shearViscosity, beta * -1.0, curcpl, updatedGeo_, true, true);
    }
    ...
    WARN("Changing coupling direction - highly experimental!!")
    // v1 p2 (now v2 p1)
    if (isMaterialComplex_)
    {
      flux1_v1_p2 = new SurfaceNitscheABInt<Complex, Complex>(new SurfaceIdentityOperator<FeH1, DIM, D_DOF>(),
                                                              new SurfaceNormalOperator<FeH1, DIM, P_DOF>(),
                                                              constOne, -1.0, curcpl, updatedGeo_, true);
    }
    else
    {
      flux1_v1_p2 = new SurfaceNitscheABInt<Double, Double>(new SurfaceIdentityOperator<FeH1, DIM, D_DOF>(),
                                                            new SurfaceNormalOperator<FeH1, DIM, P_DOF>(),
                                                            constOne, -1.0, curcpl, updatedGeo_, true);
    }

    curcpl = BiLinearForm::MASTER_SLAVE;
    // mu ( grad(v1)+grad(v1)^T ) \cdot u2 (now mu ( grad(u2)+grad(u2)^T ) \cdot v1)
    if (isMaterialComplex_)
    {
      flux2_dv1_u2 = new SurfaceNitscheABInt<Complex, Complex>(new SurfaceIdentityOperator<FeH1, DIM, D_DOF>(),
                                                               new SurfaceNormalStressOperator<FeH1, DIM, D_DOF>(subType, false),
                                                               constOne, 1.0, curcpl, updatedGeo_, true);
      flux2_dv1_u2->SetBCoefFunctionOpB(coefBB);
    }
    else
    {
      flux2_dv1_u2 = new SurfaceNitscheABInt<Double, Double>(new SurfaceIdentityOperator<FeH1, DIM, D_DOF>(),
                                                             new SurfaceNormalStressOperator<FeH1, DIM, D_DOF>(subType, false),
                                                             constOne, 1.0, curcpl, updatedGeo_, true);
      flux2_dv1_u2->SetBCoefFunctionOpB(coefBB);
    }
    ...
    // (lambda - 2/3 mu) div(v1) I \cdot u2 (now (lambda - 2/3 mu) div(u2) I \cdot v1) )
    if (isMaterialComplex_)
    {
      flux3_dv1_u2 = new SurfaceNitscheABInt<Complex, Complex>(new SurfaceIdentityOperator<FeH1, DIM, D_DOF>(),
                                                               new SurfaceNormalDivOperator<FeH1, DIM>(),
                                                               coefDivDiv, 1.0, curcpl, updatedGeo_, true);
    }
    else
    {
      flux3_dv1_u2 = new SurfaceNitscheABInt<Double, Double>(new SurfaceIdentityOperator<FeH1, DIM, D_DOF>(),
                                                             new SurfaceNormalDivOperator<FeH1, DIM>(),
                                                             coefDivDiv, 1.0, curcpl, updatedGeo_, true);
    }
    // if ( isMaterialComplex_ ) {
    ...
    if (isMaterialComplex_)
    {
      penalty_v2_u2 = new SurfaceNitscheABInt<Complex, Complex>(new SurfaceIdentityOperator<FeH1, DIM, D_DOF>(),
                                                                new SurfaceIdentityOperator<FeH1, DIM, D_DOF>(),
                                                                shearViscosity, beta, curcpl, updatedGeo_, true, true);
    }
    else
    {
      penalty_v2_u2 = new SurfaceNitscheABInt<Double, Double>(new SurfaceIdentityOperator<FeH1, DIM, D_DOF>(),
                                                              new SurfaceIdentityOperator<FeH1, DIM, D_DOF>(),
                                                              shearViscosity, beta, curcpl, updatedGeo_, true, true);
    }
    ...
    // if( use_sym_form ) {
    //   // we will deal with the out of scope stuff later
  ...
  void LinFlowPDE::DefineSurfaceIntegrators()
  {
    ...
    if (bcNode)
    {
      ...
      // omega = 2*pi*f
      PtrCoefFct omega = CoefFunction::Generate(mp_, Global::REAL, CoefXprBinOp(mp_, CoefFunction::Generate(mp_, Global::REAL, "pi*f"), CoefFunction::Generate(mp_, Global::REAL, "2"), CoefXpr::OP_MULT));

      PtrCoefFct jomega = CoefFunction::Generate(mp_, Global::COMPLEX, CoefXprBinOp(mp_, omega, CoefFunction::Generate(mp_, Global::COMPLEX, "0", "1"), CoefXpr::OP_MULT));

      PtrCoefFct omega2 = CoefFunction::Generate(mp_, Global::COMPLEX, CoefXprBinOp(mp_, omega, omega, CoefXpr::OP_MULT));
      //========================================================================================
      ...
      for (UInt i = 0; i < abcNodes.GetSize(); i++)
      {
                                                 ...
                                                 CoefXprBinOp(mp_,
                                                              ...
                                                              CoefXprBinOp(mp_,
                                                                           CoefFunction::Generate(mp_,
                                                                                                  Global::REAL, CoefXprBinOp(mp_, shearViscosity, CoefFunction::Generate(mp_, Global::REAL, "4"), CoefXpr::OP_MULT)),
        ...
        PtrCoefFct complexWaveNum2 = CoefFunction::Generate(mp_, Global::COMPLEX,
                                                            CoefXprBinOp(mp_,
                                                                         CoefFunction::Generate(mp_,
                                                                                                Global::COMPLEX, CoefXprBinOp(mp_, visc, jomega, CoefXpr::OP_MULT)),
                                                                         compressionModulus,
                                                                         CoefXpr::OP_ADD));
        ...
        PtrCoefFct complexWaveNum1 = CoefFunction::Generate(mp_, Global::COMPLEX,
                                                            CoefXprBinOp(mp_,
                                                                         omega2,
                                                                         CoefFunction::Generate(mp_,
                                                                                                Global::COMPLEX, CoefXprBinOp(mp_, CoefFunction::Generate(mp_, Global::COMPLEX, CoefXprBinOp(mp_, density, CoefFunction::Generate(mp_, Global::REAL, "-1"), CoefXpr::OP_MULT)), complexWaveNum2, CoefXpr::OP_DIV)),
                                                                         CoefXpr::OP_MULT));
        //finding c and d
        ...
        PtrCoefFct complexWaveNumRe = CoefFunction::Generate(mp_, Global::REAL, CoefXprUnaryOp(mp_, CoefFunction::Generate(mp_, Global::REAL, CoefXprBinOp(mp_, CoefFunction::Generate(mp_, Global::REAL, CoefXprBinOp(mp_, complexWaveNumAbs, complexWaveNumRe1, CoefXpr::OP_ADD)), CoefFunction::Generate(mp_, Global::REAL, "2"), CoefXpr::OP_DIV)), CoefXpr::OP_SQRT));
        //d/abs(d)
        PtrCoefFct dabsd = CoefFunction::Generate(mp_, Global::REAL, CoefXprBinOp(mp_, complexWaveNumIm1, CoefFunction::Generate(mp_, Global::REAL, CoefXprUnaryOp(mp_, complexWaveNumIm1, CoefXpr::OP_NORM)), CoefXpr::OP_DIV));
        //finding b = d/abs(d)*sqrt((-c+norm(z))/2)
        PtrCoefFct complexWaveNumIm = CoefFunction::Generate(mp_, Global::REAL, CoefXprBinOp(mp_, dabsd, CoefFunction::Generate(mp_, Global::REAL, CoefXprUnaryOp(mp_, CoefFunction::Generate(mp_, Global::REAL, CoefXprBinOp(mp_, CoefFunction::Generate(mp_, Global::REAL, CoefXprBinOp(mp_, complexWaveNumAbs, complexWaveNumRe1, CoefXpr::OP_SUB)), CoefFunction::Generate(mp_, Global::REAL, "2"), CoefXpr::OP_DIV)), CoefXpr::OP_SQRT)), CoefXpr::OP_MULT));
        //create a+bi
        PtrCoefFct complexWaveNum = CoefFunction::Generate(mp_, Global::COMPLEX, CoefXprBinOp(mp_, complexWaveNumRe, CoefFunction::Generate(mp_, Global::COMPLEX, CoefXprBinOp(mp_, complexWaveNumIm, CoefFunction::Generate(mp_, Global::COMPLEX, "0", "1"), CoefXpr::OP_MULT)), CoefXpr::OP_ADD));
        // rhoC = rho*C =sqrt(rho*k)
        PtrCoefFct rhoC = CoefFunction::Generate(mp_, Global::REAL, CoefXprUnaryOp(mp_, CoefFunction::Generate(mp_, Global::REAL, CoefXprBinOp(mp_, density, compressionModulus, CoefXpr::OP_MULT)), CoefXpr::OP_SQRT));
        //complex impedance of a viscous fluid:
        //impCoeff = rhoC/(1+real(complexWaveNum)/complexWaveNum)
        PtrCoefFct impCoeff = CoefFunction::Generate(mp_, Global::COMPLEX, CoefXprBinOp(mp_, rhoC, CoefFunction::Generate(mp_, Global::COMPLEX, CoefXprBinOp(mp_, CoefFunction::Generate(mp_, Global::REAL, "1"), CoefFunction::Generate(mp_, Global::COMPLEX, CoefXprBinOp(mp_, complexWaveNumRe, complexWaveNum, CoefXpr::OP_DIV)), CoefXpr::OP_ADD)), CoefXpr::OP_DIV));
