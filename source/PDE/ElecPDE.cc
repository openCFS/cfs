...
//          WARN(warnmsg.str());
  ...
  void ElecPDE::InitTimeStepping()
  {
    if (GetDomain()->GetAdaptiveData())
        EXCEPTION("Adaptive timestepping is not supported for ElecPDE: variable-step BDF2 requires C3-smooth solution history, which is not registered for this PDE. Use fixed deltaT.");
