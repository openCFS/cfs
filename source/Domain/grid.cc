...
void Grid::AddRegions( const StdVector<std::string> & regionNames, StdVector<
    RegionIdType> & rIds ) {
...
void Grid::RegionNameToId( StdVector<RegionIdType> & regionIds,
                           const StdVector<std::string> & regionNames ) {
...
shared_ptr<EntityList> Grid::GetEntityList( EntityList::ListType listType,
  ...
  if( listType == EntityList::ELEM_LIST ) {
    ...
    shared_ptr<NodeList> nodeList =
        shared_ptr<NodeList> ( new NodeList( this ) );
...
void Grid::Dump( ) {
  ...
  std::cout << "Grid: elements=" << GetNumElems() << " nodes=" << GetNumNodes()
      << std::endl;
  ...
  for( UInt i = 0; i < regionNames_.GetSize(); i++ ) {
    ...
    std::cout << "region: " << region_name << " id=" << region_id
        << " elements=" << elems.GetSize() << std::endl;
  }
}
...
bool Grid::InitNonmatchingInterfaces( ) {
  ...
  if( !pNode ) {
    ...
    LOG_DBG2(grid)
<<    "NonMatching: No non-conforming interfaces found!";
    return true;
  }
