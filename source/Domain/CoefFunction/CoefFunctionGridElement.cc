...
//================================================================================================
/*!
 *       \file     CoefFunctionGridElement.cc
 *       \brief    Implementation of element wise grid inteprolation
 *
...
//DECLARE_LOG(CoefFunctionGridElement)
//DEFINE_LOG(CoefFunctionGridElement, "CoefFunctionGridElement")

template<class DATA_TYPE>
  CoefFunctionGridElement<DATA_TYPE>::CoefFunctionGridElement(Domain* ptDomain,
  ...
  //! Destructor
  template<class DATA_TYPE>
  CoefFunctionGridElement<DATA_TYPE>::~CoefFunctionGridElement(){
#ifdef USE_OPENMP
  ...
  std::string OneFactorStringFromMultipleFactor(StdVector<std::string>& factors) {
  ...
  template<class DATA_TYPE>
  shared_ptr<CoefFunction> CoefFunctionGridElement<DATA_TYPE>::CreateFactorFunction(std::string factorString){
  ...
  template<class DATA_TYPE>
  CoefFunction::CoefDependType CoefFunctionGridElement<DATA_TYPE>::ReadFactorFunctionStringDependency(std::string factorString){
    shared_ptr<CoefFunction> function = this->CreateFactorFunction(factorString);
    return function->GetDependency();
  }
  
  template<class DATA_TYPE>
  void CoefFunctionGridElement<DATA_TYPE>::WriteGlobalFactorsToXML(PtrParamNode configNode){
  ...
  template<class DATA_TYPE>
  void CoefFunctionGridElement<DATA_TYPE>::InitGlobalFactorFunctions(PtrParamNode configNode){
    ...
    if (hasSpaceFactor_) {
      std::string factorString = OneFactorStringFromMultipleFactor(spaceFactors);
      ...
      std::string factorString = OneFactorStringFromMultipleFactor(constantFactors);
    ...
    if (hasTimeFreqFactor_) {
      std::string factorString = OneFactorStringFromMultipleFactor(timeFreqFactors);
    ...
    if (hasGeneralFactor_) {
      std::string factorString = OneFactorStringFromMultipleFactor(generalFactors);
  ...
  template<class DATA_TYPE>
  void CoefFunctionGridElement<DATA_TYPE>::GetTensor(Matrix<DATA_TYPE>& CoefMat,
                                                   const LocPointMapped& lpm ){
    EXCEPTION("GetTensor is not implemented here"); 
  }

  template<class DATA_TYPE>
  void CoefFunctionGridElement<DATA_TYPE>::GetVector(Vector<DATA_TYPE>& CoefMat,
                                                   const LocPointMapped& lpm ){
    EXCEPTION("GetVector is not implemented here"); 
  }

  template<class DATA_TYPE>
  void CoefFunctionGridElement<DATA_TYPE>::GetScalar(DATA_TYPE& CoefMat,
                                                   const LocPointMapped& lpm ){
  ...
  template<class DATA_TYPE>
  void CoefFunctionGridElement<DATA_TYPE>::CreateOperator(UInt spaceDim, UInt dofDim){
    if(dofDim == 1){
      if(spaceDim == 2)
  ...
  template<class DATA_TYPE>
  void CoefFunctionGridElement<DATA_TYPE>::CreateDivOperator(UInt spaceDim, UInt dofDim){
    if(spaceDim != dofDim)
      EXCEPTION("CoefFunctionGridElement<DATA_TYPE>: Divergence need vectorial data!");
  ...
  template<class DATA_TYPE>
  void CoefFunctionGridElement<DATA_TYPE>::MapEqns(){
  ...
  template<class DATA_TYPE>
  void CoefFunctionGridElement<DATA_TYPE>::InitSolVec(){
  ...
  template<class DATA_TYPE>
  void CoefFunctionGridElement<DATA_TYPE>::GetElemSolution(Vector<DATA_TYPE> & sol, UInt eNum){
    StdVector<UInt> connect;
    srcGrid_->GetElemNodes(connect,eNum);
    sol.Resize(dimDof_);
// todo #4.1 the solution is already on the element
    for(UInt d = 0; d<dimDof_;++d){
      //sol[d] = solVec_[(eNum-2167)*dimDof_+d];
// todo #4.2 somehow we have to check how to get the offset
...
//    }
  ...
  template<class DATA_TYPE>
  void CoefFunctionGridElement<DATA_TYPE>::InitRegionNodes(){
  ...
  template<class DATA_TYPE>
  void CoefFunctionGridElement<DATA_TYPE>::InitRegionNodeCoordinates(){
...
#pragma omp parallel for      
  ...
  template<class DATA_TYPE>
  void CoefFunctionGridElement<DATA_TYPE>::ClearRegionNodeCoordinates(){
    initializedRegionNodeCoordinates_ = false;
    regionNodeCoordinates_ = StdVector<StdVector<Vector<Double> > >();
  }
  
  template<class DATA_TYPE>
  void CoefFunctionGridElement<DATA_TYPE>::InitSpaceFactor(){
  ...
  template<class DATA_TYPE>
  void CoefFunctionGridElement<DATA_TYPE>::InitUsedRegionNodesForSum(){
  ...
  void CoefFunctionGridElement<DATA_TYPE>::CopyResult(Vector<DATA_TYPE>& sol,
...
#pragma omp critical
  ...
  template<class DATA_TYPE>
  void CoefFunctionGridElement<DATA_TYPE>::ReadSolution(UInt step, Double stepValue, Vector<DATA_TYPE> & sol){
  ...
  template<class DATA_TYPE>
  void CoefFunctionGridElement<DATA_TYPE>::ReadSolution(UInt step,Vector<DATA_TYPE> & sol){
    ...
    typename CoefFunctionGridElement<DATA_TYPE>::CopyResultFunction::Ptr crf = GetCopyResultFunction(
...
#pragma omp parallel for
        ...
        //todo #3.1 just load the vector, do not consider what
        sol = resVec;
//        crf(sol,resVec,nodeNums,generalFactor,spaceFactor,constantFactor,loadedSum,factorSum,countNodes);
  ...
  template<class DATA_TYPE>
  bool CoefFunctionGridElement<DATA_TYPE>::UpdateSolution(){
    ...
    if (this->snapToCFSStep_) {
      //todo #3.2 only consider snapToTimeSteps
...
#pragma omp parallel for
  ...
  template class CoefFunctionGridElement<Double>;
  template class CoefFunctionGridElement<Complex>;
#endif
