...
//================================================================================================
/*!
 *       \file     CoefFunctionGridElement.hh
...
#ifndef COEFFUNCTIONGRIDELEMENENT_HH
#define COEFFUNCTIONGRIDELEMENENT_HH
...
class CoefFunctionGridElement : public CoefFunctionGrid{
  public:

    ///Constructor with the configuration xml node reading the type of interpolation
    CoefFunctionGridElement(Domain* ptDomain, PtrParamNode configNode, shared_ptr<RegionList> regions);

    ///Destructor freeing al used data strutures
    virtual ~CoefFunctionGridElement();
  ...
  protected:
    ...
    template<bool useSpaceFactorA, bool useSpaceFactorB, bool useConstFactor, 
        bool countSum, bool countAllValues, UInt dimDof>
    static typename CoefFunctionGridElement<DATA_TYPE>::CopyResultFunction::Ptr GetCopyResultFunction() {
      return &CoefFunctionGridElement<DATA_TYPE>::CopyResult<
    ...
    static typename CoefFunctionGridElement<DATA_TYPE>::CopyResultFunction::Ptr GetCopyResultFunction(const UInt dimDof) {
      if (dimDof == 1) { // scalar
      ...
      EXCEPTION("Cannot find CopyResult function for dimension " << dimDof << " in CoefFunctionGridElement::GetCopyResultFunction");
      return GetCopyResultFunction<useSpaceFactorA,useSpaceFactorB,useConstFactor,countSum,countAllValues,1>();
    }
