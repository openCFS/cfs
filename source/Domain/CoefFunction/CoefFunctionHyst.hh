...
//        std::cout << "However, no coupling was defined in .xml file." << std::endl;
  ...
  public:
    ...
    void GetTensor(Matrix<Double>& outputTensor, const LocPointMapped& lpm ){
      ...
      // > moved inside computeTensor as there are different timeLevels for diff now
      
      hystHelper_->ComputeTensor(outputTensor, lpm, tensorName_, implementationVersion, transposed_, rotate, useAbs );
  ...
  public:
    ...
    void GetTensor(Matrix<Double>& outputTensor, const LocPointMapped& lpm ){
      ...
      if( (resultName_ == "DeltaPermeability")||(resultName_ == "DeltaPermittivity") ){
        ...
        hystHelper_->ComputeTensor(outputTensor,lpm, resultName_, "None", false, false, false, false );
      }
    }
    ...
    Matrix<Double> transformPreisachWeightsForIsotropicVectorCase(ParameterPreisachWeights* paramSet){
      ...
      for(UInt i = dimHalfInt; i < numRows; i++){
        ...
        for(UInt k = numRows-1-i; k <= i; k++){
          ...
          if(alpha == 0){
            vectorWeights[i][k] = 0.75*getWeight(paramSet, 0, 0, delta);
          } else {
            lambda = beta/alpha;
      ...
      for(UInt k = 0; k < dimHalfInt; k++){
        ...
        for(UInt i = k; i < numRows-k-1; i++){
          ...
          if(beta == 0){
            vectorWeights[i][k] = 0.75*getWeight(paramSet,alpha, beta, delta);
          } else {
            // flipped case
            lambda = alpha/beta;
            vectorWeights[i][k] = 0.0;
