...
BaseNcInterface::BaseNcInterface(Grid* grid) :
  ptGrid_(grid),
  name_(""),
  region_(NO_REGION_ID),
  masterSurfRegion_(NO_REGION_ID),
  slaveSurfRegion_(NO_REGION_ID),
  masterVolRegion_(NO_REGION_ID),
  slaveVolRegion_(NO_REGION_ID),
  isMoving_(false),
  coordSysId_(""),
  coordSys_(NULL),
  ...
  tolRel_(1e-4)
{
  ...
  mphOffset_[0] = mParser_->GetNewHandle(true); 
  mphOffset_[1] = mParser_->GetNewHandle(true); 
  mphOffset_[2] = mParser_->GetNewHandle(true); 
};
...
BaseNcInterface::~BaseNcInterface() {
  ...
  mParser_->ReleaseHandle(mphOffset_[0]);
  mParser_->ReleaseHandle(mphOffset_[1]);
  mParser_->ReleaseHandle(mphOffset_[2]);
  mParser_ = NULL;
}

void BaseNcInterface::SetName(const std::string &name) {
  if ( name_ != "") {
    EXCEPTION("You cannot rename a NcInterface at runtime.");
  }
  if ( name == "" ) {
    EXCEPTION("Empty string is not a valid name for a NcInterface.");
  }
...
void BaseNcInterface::SetMasterSurfRegion(RegionIdType region) {
  ...
  if ( surfRegionIds.Find(region) != surfRegionIds.End() ) {
    masterSurfRegion_ = region;
  } else {
    EXCEPTION("Could not find surface region ID " << region);
  }
}

void BaseNcInterface::SetSlaveSurfRegion(RegionIdType region) {
  if ( region == NO_REGION_ID ) {
    slaveSurfRegion_ = NO_REGION_ID;
    return;
  }
  ...
  if ( surfRegionIds.Find(region) != surfRegionIds.End() ) {
    slaveSurfRegion_ = region;
  } else {
    EXCEPTION("Could not find surface region ID " << region);
  }
}

void BaseNcInterface::SetMasterVolRegion(RegionIdType region) {
  if ( region == NO_REGION_ID ) {
    masterVolRegion_ = NO_REGION_ID;
    return;
  }
  
  StdVector<RegionIdType> volRegionIds;
  ptGrid_->GetSurfRegionIds(volRegionIds);
  
  if ( volRegionIds.Find(region) != volRegionIds.End() ) {
    masterVolRegion_ = region;
  } else {
    EXCEPTION("Could not find volume region ID " << region);
  }
}

void BaseNcInterface::SetSlaveVolRegion(RegionIdType region) {
  if ( region == NO_REGION_ID ) {
    slaveVolRegion_ = NO_REGION_ID;
    return;
  }
  ...
  if ( volRegionIds.Find(region) != volRegionIds.End() ) {
    slaveVolRegion_ = region;
  } else {
    EXCEPTION("Could not find volume region ID " << region);
  }
}

void BaseNcInterface::SetStationary() {
  isMoving_ = false;
  coordSysId_ = "";
  coordSys_ = NULL;
}

void BaseNcInterface::SetRotation(const std::string &coordSysId, Double rpm) {
  if ( rpm == 0.0 ) {
    SetStationary();
    return;
  }
  ...
  offsetExpr_.Resize(3);
  offsetExpr_[1] = sstr.str();
  
  mParser_->SetExpr(mphOffset_[1], offsetExpr_[1]);
}

void BaseNcInterface::SetMotion(const StdVector<std::string> &offsetExpr,
...
bool BaseNcInterface::SideOnSide (SurfElem *ifaceElem1, SurfElem *ifaceElem2,
                       ...
                       StdVector<UInt> &newNodes )
{
  ...
  ptGrid_->GetNodeCoordinate(c0, nodenum_c0, coordUpdate);
  ptGrid_->GetNodeCoordinate(c1, nodenum_c1, coordUpdate);
  ptGrid_->GetNodeCoordinate(d0, nodenum_d0, coordUpdate);
  ptGrid_->GetNodeCoordinate(d1, nodenum_d1, coordUpdate);
...
bool BaseNcInterface::RectangleOnRectangle(SurfElem *ifaceElem1, SurfElem *ifaceElem2,
                                bool coordUpdate, StdVector<NcSurfElem*>& elemList)
{
  ...
  // Get coordinates of the endpoints
  ptGrid_->GetNodeCoordinate(c0, ifaceElem1->connect[0], coordUpdate);
  ptGrid_->GetNodeCoordinate(c1, ifaceElem1->connect[1], coordUpdate);
  ptGrid_->GetNodeCoordinate(c2, ifaceElem1->connect[3], coordUpdate);
  ptGrid_->GetNodeCoordinate(d0, ifaceElem2->connect[0], coordUpdate);
  ptGrid_->GetNodeCoordinate(d1, ifaceElem2->connect[1], coordUpdate);
  ptGrid_->GetNodeCoordinate(d2, ifaceElem2->connect[2], coordUpdate);
...
bool BaseNcInterface::PolygonOnPolygon(SurfElem *ifElem1, SurfElem *ifElem2,
...
UInt p1Size, p2Size;
  ...
  p1.Resize(p1Size);
  for (i = 0; i < p1Size; ++i)
    ptGrid_->GetNodeCoordinate(p1[i], ifElem1->connect[i], coordUpdate);
  ...
  p2.Resize(p2Size);
  for (i = 0; i < p2Size; ++i)
    ptGrid_->GetNodeCoordinate(p2[i], ifElem2->connect[i], coordUpdate);
...
BaseNcInterface::LineIntersectType BaseNcInterface::CutLines(const Vector<Double> &a,
...
bool BaseNcInterface::CutPolys(StdVector< Vector<Double> > &p1,
...
bool BaseNcInterface::PointInsidePoly(const Vector<Double> &p,
...
Double BaseNcInterface::PolyCentroid(const StdVector< Vector<Double> > &p,
...
UInt BaseNcInterface::TriangulatePoly(const StdVector< Vector<Double> > &p,
