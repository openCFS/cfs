...
#include "PDE/MagEdgeMixedGaugedAPDE.hh"
...
void Domain::CreateSinglePDEs(UInt sequenceStep, PtrParamNode infoNode)
{
  ...
  for (UInt i = 0; i < pdeNodes.GetSize(); i++)
  {
    ...
    if (actPdeName == "electrostatic")
      ...
      }else if(formulation == "mixedGaugedA"){
          std::cout<<"making mixedGaugedA"<<std::endl;
          ptSinglePde_[i] = new MagEdgeMixedGaugedAPDE(defaultGrid, actPdeNode, infoNode, simState_, this);
      }else{
          EXCEPTION("Formulation of MagEdgePDE not known!");
      }
    }
