	...
	for (j=1 ; j<=6; j++)
	  {
	    *strPtr >> helpval;
	    ...
	    //	    material -> SetMatrixData(i,j, helpval);
	    material -> SetMatrixData(i-1, j-1, helpval);
	  }
	delete strPtr;	
      }
	...
	for (j=1 ; j<=6; j++)
	  {
	    *strPtr >> helpval;
	    ...
	    material -> SetMatrixData(i+6-1,j-1, helpval);
	    material -> SetMatrixData(j-1,i+6-1, helpval);     // writes transposed coupling terms
	    //	    material -> SetMatrixData(i+6,j, helpval);
	    //	    material -> SetMatrixData(j,i+6, helpval);     // writes transposed coupling terms
	  }
	delete strPtr;	
      }
	...
	for (j=1 ; j<=3; j++)
	  {
	    *strPtr >> helpval;
	    ...
	    //	    material -> SetMatrixData(i+6-1,j+6-1, helpval);
