...
//     indexSetCountFileIn->close();
  ...
  void MHMaterialDataFile::computeIndexSet(Integer PP, UInt PPmax, UInt nrMultHarms_){
    ...
    Integer indexset[6][15][400][9];

    std::cout<<" Init IndexSet:" <<std::endl;
//    Integer**** indexset;
//     indexset = new Integer***[PPmax+2];
//     indexset[0] = new Integer**[4*nrMultHarms_+2];
//     indexset[0][0] = new Integer*[countmax];
//     indexset[0][0][0] = new Integer[2*nrMultHarms_+2];

//     std::cout<<" Init IndexSet 1 :" <<std::endl;
//      for(UInt k=1;k<countmax;k++)
//        indexset[0][0][k]=indexset[0][0][k-1]+(2*nrMultHarms_+2);

//      std::cout<<" Init IndexSet 2 :" <<std::endl;
//      for(UInt i=0;i<PPmax+2;i++)
//        for(UInt k=1;k<4*nrMultHarms_+2;k++)
//          indexset[i][k]=indexset[i][k-1]+(countmax*(2*nrMultHarms_+2));
     
//     std::cout<<" Init IndexSet 3 :" <<std::endl;
//      for(UInt k=1;k<PPmax+2;k++)
//        indexset[k]=indexset[k-1]+(4*nrMultHarms_+2);
     //       indexset[k]=indexset[k-1]+((4*nrMultHarms_+2)*countmax*(2*nrMultHarms_+2));

//      for(UInt k=1;k<countmax+2;k++)
//        indexset[0][k]=indexset[0][k-1]+(4*nrMultHarms_+2);


//     for (UInt k = 0; k < (PPmax+2) * 10; k++) 
//       index_[0][k]=0;


    for (UInt i=0;i<PPmax+2;i++)
      for (UInt j=0;j<4*nrMultHarms_+2;j++)
        for (UInt k=0;k<UInt(countmax);k++)
           for (UInt l=0;l<2*nrMultHarms_+2;l++)
             indexset[i][j][k][l]=0;
  ...
  void MHMaterialDataFile::getExponentArray(Matrix<Integer> & exponent, 
                                            Matrix<Integer> & count, UInt PP,
                                            UInt p, Integer delta)
  {
    ENTER_FCN("MultiHarmonicDriver::getExponentArray");

    std::cout<<" ++ MultiHarmonicDriver::getExponentArray"<<std::endl;
...
//     UInt nrOfOcc=0;
//     for (UInt kk=0;kk<4*nrMultHarms_+2;kk++)
//       nrOfOcc+=count[p][kk];
//     std::cout<<"nrOfOcc:"<<std::endl;
//     std::cout<<nrOfOcc<<std::endl;
//     getchar();
        ...
        if(pNum==p){
          ...
          do{
            ...
            if (deltaNum==delta+2*nrMultHarms_){
              //              std::cout<<"deltaNum = " <<deltaNum<<std::endl;
              UInt counter=0;
              UInt expCounter=0;
              while(mDataRow[2+counter]!=','){
                if ((mDataRow[2+counter]!=',')&&(mDataRow[2+counter]!=' ')){
//                   std::cout<<"exponent["<<cCounter<<"]["<<expCounter<<"] = "<<(mDataRow[2+counter])-48<<std::endl;
 ...
 void MHMaterialDataFile::readData(){
    ENTER_FCN( "mhMaterialDataFile::readData" );
...
//       }
   ...
   //  std::cout<<"ReadData  parameter = "<< std::endl;
