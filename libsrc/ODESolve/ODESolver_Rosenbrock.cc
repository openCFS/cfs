...
namespace CoupledField
{
  ...
  void ODESolver_Rosenbrock::RosenbAdaptiveStepsize (StdVector<Double> &y,
	...
	err[i]=E1*g1[i]+E2*g2[i]+E3*g3[i]+E4*g4[i];
      ...
      t= xsav+h;
      if (t == xsav){
	Error("Stepsize not significant in stiff",__FILE__,__LINE__);
      }
      errMax = 0.0;
      for(i=0; i<n; i++){
    ...
    Error("Exceeded maxTry in stiff",__FILE__,__LINE__);
  }
  ...
  void ODESolver_Rosenbrock::LUDecomposition(Matrix<Double> &a,
	...
	if ( (temp=fabs(a(i,j))) > big ) big=temp;
      ...
      if ( big == 0){
	Error( "Singular matrix in routine LUDecomposition", __FILE__, __LINE__ );
      }
      v[i]= 1.0 / big;
    }
