...
namespace CoupledField{
  ...
  typedef enum { NOSOLUTIONTYPE, COUPLING_TYPE,
		 ...
		 MECH_FORCE, ELEC_POTENTIAL, ELEC_FIELD, ELEC_FORCE,
		 SMOOTH_DISPLACEMENT, ACOU_POTENTIAL, ACOU_VELOCITY,
		 ACOU_PRESSURE, ACOU_FORCE, MAG_POTENTIAL, MAG_FIELD,
		 MAG_EDDY_CURRENT, MAG_FORCE} SolutionType;
  ...
  public:
    ...
    //! \note All entries of this object are deleted
    virtual void SetDof(const Integer dof,
			const SolutionType sol = NOSOLUTIONTYPE) = 0;
    ...
    virtual void GetSolVector(const SolutionType type, CFSVector & val)
      const = 0;
    ...
    virtual void GetSolVectorSingleDof(const SolutionType type,
				       const Integer dof,
				       CFSVector & val) const = 0;
    ...
    virtual void GetSolution(const SolutionType type,
			     BaseStoreSol & val) const = 0;

    //! Get a single entry
    //! \NOTE: This method must only be used with solutions with only one
    //! entrytype
    virtual void Get(const Integer nodeNr, const Integer dof,
		     Double & ret) const
    {Error("BaseStoreSol::Get() not implemented here",__FILE__,__LINE__);} 

    //! Get a single entry of a given solutiontype and a given dof
    virtual void Get(const SolutionType type, const Integer nodeNr,
		     const Integer dof, Double & ret) const
    {Error("BaseStoreSol::Get() not implemented here",__FILE__,__LINE__);}

    //! Set a single entry of a given solutiontype and a given dof
    virtual void Set(const SolutionType type, const Integer nodeNr,
		     const Integer dof, const Double val)
    ...
    virtual void Add(const SolutionType type, const Integer nodeNr,
		     const Integer dof, const Double val) const
