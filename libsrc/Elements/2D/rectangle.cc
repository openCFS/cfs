...
void Rectangle :: SetDerTransformFncAtIntPoints3D()
{
#ifdef TRACE
  ...
  DetaTransFnc4AtIP.Resize(NumIntPoints);
  ...
  for (i=0; i < NumIntPoints; i++)
    { 
      ...
      DetaTransFnc4AtIP[i]=TransFnc4deta(IntPoints[i][0],IntPoints[i][1],IntPoints[i][2]);
    }
}
                     ...
                     Point<3> * ptCoord, const Boolean NeedJinv)
{
 ...
 J.J[0][1] = DetaTransFnc1AtIP[ip]*ptCoord[0][0] + DetaTransFnc2AtIP[ip]*ptCoord[1][0]
          + DetaTransFnc3AtIP[ip]*ptCoord[2][0] + DetaTransFnc4AtIP[ip]*ptCoord[3][0];
 ...
 J.J[1][1] = DetaTransFnc1AtIP[ip]*ptCoord[0][1] + DetaTransFnc2AtIP[ip]*ptCoord[1][1]
          + DetaTransFnc3AtIP[ip]*ptCoord[2][1] + DetaTransFnc4AtIP[ip]*ptCoord[3][1];
 ...
 J.J[2][1] = DetaTransFnc1AtIP[ip]*ptCoord[0][2] + DetaTransFnc2AtIP[ip]*ptCoord[1][2]
          + DetaTransFnc3AtIP[ip]*ptCoord[2][2] + DetaTransFnc4AtIP[ip]*ptCoord[3][2];
