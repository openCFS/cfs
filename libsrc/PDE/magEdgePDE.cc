...
#ifdef DEBUG
	  (*cla) << "Edge Nr. and signs: Element  " <<   j+1 << std::endl;
	  (*cla) << epos << std::endl;
	  (*cla) << esign << std::endl;
#endif
	  // stiffness part
...
#ifdef DEBUG
	  (*cla) << "Stiffnessmatrix, ElementNumber  " <<   j+1 << std::endl;
	  (*cla) << elemmat << std::endl;
#endif
...
#ifdef DEBUG
	  (*cla) << "Mass, ElementNumber  " <<   j+1 << std::endl;
	  (*cla) << elemmat << std::endl;
#endif
...
#ifdef DEBUG
	  (*cla) << "Auxilarymatrix, ElementNumber  " <<   j+1 << std::endl;
	  (*cla) << elemmat_aux << std::endl;
#endif
...
#ifdef DEBUG
	  (*cla) << "Edge Nr.to Algsys, Element: " <<   iel+1 << std::endl;
	  (*cla) << epos << std::endl;
#endif
	}
    }
...
//   if (conf->ifgetliststr("Coil", coil, "magnetic"))
  ...
  for (Integer actCoil=0; actCoil < coilDomain_.size(); actCoil++)
	...
	{	  
	  ...
	  if (InfoPrint)
	    *infofile << " COIL: domain "  << subdoms_[i] << std::endl
		      << "       current density: "  << currentDens << std::endl
		      << "       currDirection:   "  << currDirection << std::endl;
	  ...
	  for (j=0; j < elemssd.size(); j++)
	    {  
	      ...
	      BaseForm * rhsSource = new LinearEdgeInt(ptElem, currentDens, currDirection);
		...
		elemVec[ii] *= esign[ii];
	      ...
	      (*cla) << epos << std::endl;
	      (*cla) << esign << std::endl;
	      (*cla) << "RHS, ElementNumber  " << j+1 << std::endl;
	      (*cla) << elemVec << std::endl;
#endif
...
void MagEdgePDE::PostProcess(const Integer level)
{
  ...
  magVecPot_ = new Vector<Double>[dim];
  // Resize solution arrays
  for( Integer i=0; i<dim; i++)
    magVecPot_[i].Resize(NumElems_);  
  ...
  for (Integer isd=0; isd<subdoms_.size(); isd++)
      ...
      for (Integer iel=0; iel< elemssd.size(); iel++)
	{
 	  ...
 	  magField->CalcElemMagVec(elemMagVecPot, elemssd[iel], lCoord);

	  for (Integer k=0; k<dim; k++)
	    {
	      bField_[k][actEl]    = elemMagField[k];
	      magVecPot_[k][actEl] = elemMagVecPot[k];
	    }
...
#ifdef DEBUG
	  (*debug) << "Reading coil domain: " << coilDomain_[i] << std::endl
		   << "     values: iDir    = " << coilDef_[i].iDir << std::endl
