...
void Acou2dFlowNoise::SetMatrixFactors()
{
#ifdef TRACE
  (*trace) << "entering Acou2dFlowNoise::SetMatrixFactors" << std::endl;
#endif
...
void Acou2dFlowNoise::SpecifyMatrices(Integer &matrixtype, Integer * matrixsystype, Integer &graphtype, Integer &numdofpernode, Integer &numdirichlets,
Integer &numconstraints)
{
#ifdef TRACE
  (*trace) << "entering Acou2dFlowNoise::SpecifyMatrices" << std::endl;
#endif
...
void Acou2dFlowNoise::SetupMatrices(const Integer level, BCs * ptBCs)
{
#ifdef TRACE
  (*trace) << "entering Acou2dFlowNoise::SetupMatrices" << std::endl;
#endif
  ...
  conf->getliststr("domain_bnd",domain_surfaces_,"Acoustic");
  DomainBnd=ptBCs->getEdgesBC(domain_surfaces_[0],level);
...
#ifdef TRACE
  (*trace) << "Leaving Acou2dFlowNoise::SetupMatrices" << std::endl;
#endif
}
...
void Acou2dFlowNoise::ComputeRHS(const Double atime, BCs * ptBCs)
{
  ...
  Next2Surf=ptBCs->getNeighbors2DElemsFor1D(rhs_surfaces_[1],level);
  ...
  ///std::cout << "Time: "<< atime << std::endl; 
    ...
    // 	    {
    // 	      ptEl=elemssd[j]->ptElem;
    // 	      BaseForm<Point<2>> * linear_load = new LinearForm<Point<2>>(ptEl);
...
void Acou2dFlowNoise::CalcCoeff(Vector<Double> & coeffmass, Vector<Double> & coeffstiff, Vector<Double> & coeffdamp)
{
