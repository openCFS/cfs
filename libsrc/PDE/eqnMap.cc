...
namespace CoupledField {
  ...
  void EqnMap::Finalize() {
    ...
    for ( it=resEntMap_.begin(); it!=resEntMap_.end(); it++ ) {
	  ...
	  it->first.definedOn == ResultDof::PFEM ) {
    ...
    for ( it=resEntMap_.begin(); it!=resEntMap_.end(); it++ ) {
      // check if resultDof is mapped onto nodes
      if( it->first.definedOn == ResultDof::EDGE ||
	  it->first.definedOn == ResultDof::PFEM ) {
	for( UInt iList = 0; iList < it->second.GetSize(); iList++ ) {
    ...
    if( ptGrid_->GetDim() == 3 ) {
      ...
      for ( it=resEntMap_.begin(); it!=resEntMap_.end(); it++ ) {
        // check if resultDof is mapped onto nodes
        if( it->first.definedOn == ResultDof::FACE ||
            it->first.definedOn == ResultDof::PFEM ) {
          for( UInt iList = 0; iList < it->second.GetSize(); iList++ ) {
    ...
    for( it = resEntMap_.begin(); it != resEntMap_.end(); it++ ) {
      ...
      if( it->first.definedOn == ResultDof::PFEM ) {
        for( UInt iList = 0; iList < it->second.GetSize(); iList++ ) {
  ...
  void EqnMap::GetEqns( StdVector<Integer> &eqns,
    ...
    if ( result.definedOn == ResultDof::NODE ||
	 result.definedOn == ResultDof::PFEM ) {
    ...
    if ( result.definedOn == ResultDof::EDGE ||
	 result.definedOn == ResultDof::PFEM ) {
    ...
    if ( result.definedOn == ResultDof::FACE ||
	 result.definedOn == ResultDof::PFEM ) {
    ...
    //  ELEM PART
    // =============
    if( result.definedOn == ResultDof::ELEMENT  ||
        result.definedOn == ResultDof::PFEM ) {
