...
\item \textbf{static}
\item \textbf{harmonic}
\item \textbf{multiSequence}
\item \textbf{paramIdent}
\item \textbf{eigenfrequency}\footnote{This is currently not implemented.}
\end{itemize}
...
<multiSequence>
  <step>
    <pde type="electrostatic" analysis="static" tag="elecStep1"/>
    <pde type="mechanic" analysis="static" tag="mechStep1"/>
  </step>
  <step>
    <pde type="electrostatic" analysis="transient" tag="elecStep2"/>
    <pde type="mechanic" analysis="transient" tag="mechStep2"/>
  </step>
</multiSequence>
\end{verbatim}
      ...
      occurence. The \xml{tag} attribute specified in each \xml{pde}
      subelement is a symbolic name that should be unique. It is used to
      locate the appropriate boundary conditions or transient parameters etc.
      for this step in the file.
\item In order to specify for each step different boundary conditions or
      ...
      \xml{tag} attribute. In order to specify different parameters for
      different steps the element must be given several times and the
      \xml{tag} must be chosen accordingly.
