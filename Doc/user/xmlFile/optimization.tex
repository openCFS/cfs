...
The base element for all optimization tasks is the \verb#<optimization>#
element. If present, optimization is enabled and the PDE problem defined in the
\verb#<sequenceStep># will be evaluatated multiple times within the iterations
of the optimization process.

\subsubsection{Logging}
Key properties for the itations, like objective and constraint values are logged
to the \textbf{.info.xml} file. Additionally the same information can be logged
to a gnuplot compatible text file. This allows to plot graphs during
the optimization process. Assign an arbitraty name by
\begin{verbatim}
<optimization log="myplot.plot.dat">.
\end{verbatim}
If the log name is \textbf{``[problem]''} or \textbf{``[true]''} (default), the
actual problem name with the extension \textbf{``.plot.dat''} is chosen. Only
the value \textbf{``false''} disables writing of the additional log file.
