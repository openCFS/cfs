...
\begin{itemize}
\item A simple but complete description of what to do with the system of a direct coupled PDE. The format is by default \xml{matrix-market}, hence the files 'piezo.mm' and 'piezo.vec' with the RHS are written. 
\begin{verbatim}
<linearSystems>
   <system name="direct">
...
\end{verbatim}
\item In this example only 'piezo.sol.vec' is created. This might be usefull if one wants to compare
the solutions of different solvers (or solver settings). The \xml{format} attribute applies only for the system and is therefore ignored. The value 'matrix-market' would be default anyway.
\begin{verbatim}
...
\item If you have a positive definite system (real symmetric or Hermitian), you have to give this information explicitly, otherwise an indefinite symmetric system is assumed and some performance might be lost. The shortest version of \xml{linearSystems} is then like the following:
\begin{verbatim}
<linearSystems>
  <system name="mechanic">
     <solver type="ilupack">
...
\end{verbatim}
\item If the iterative solver from the ILUPACK package exits too early, give a stricter \xml{residualTol}. 
\begin{verbatim}
<solver type="ilupack">
   <ilupack>
...
\end{verbatim}
Giving more parameters one can try to increase performance. Like in the following example, where we explicitly express that the system is positive definit and we want logging to find hints for further tuning. Note that the pizeo example from above would not be positive definite.
\begin{verbatim}
<linearSystems>
  <system name="mechanic">
    <solver type="pardiso">
