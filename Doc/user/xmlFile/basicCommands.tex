...
\end{figure}
If no further option is specified, the material coordinate directions $(1,2,3)$ get associated with the global cartesian direction $(x,y,z)$. In a 2D geometry, only the first two components are taken $(1,2)$.
...
\end{verbatim}
The previous example would e.g. rotate the (material-) local $3$-direction by $\alpha=-90^\circ$ around the local $1$-axis, so that it coincides with the global $y$-direction. This is needed, if for example a plane simulation of a piezoelectric material has to be performed, where the polarization is given in local $3$-direction, whereas the geometric model assumes a polarization in $y$-direction.
\newline
...
% use packages: array
\begin{tabular}{c|ccc}
 & \multicolumn{3}{x}{Local Coordinate system} \\
...
\end{verbatim}
Here the material PZT-4 is associated with a previously defined coordinate system \verb#myPoar1#.
...
Putting both definitions together, the material axes $(1,2,3)$ are associated with the cyldindric components $(r,\varphi,z)$in the following way
\begin{equation}
\left( 
\begin{array}{c}
  1 \\ 2 \\ 3 
